...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	writeln!(embedded_file, "// This file contains embedded English locale files")?;
	writeln!(embedded_file)?;
	// No imports needed for match-based lookup
	writeln!(embedded_file)?;

	// Generate optimized lookup function instead of HashMap
	writeln!(embedded_file, "pub fn get_embedded_locale(key: &str) -> Option<&'static str> {{")?;
	writeln!(embedded_file, "    match key {{")?;

	// Try to detect if we're building for a specific utility by checking build
	// configuration This attempts to identify individual utility builds vs
	// multicall binary builds
...
/// or if the current directory structure does not allow determining the project
/// root.
...
fn embed_single_utility_locale(
	...
	if util_name.ends_with("sum") {
		embed_component_locales(embedded_file, locales_to_embed, "checksum_common", |locale| {
			project_root.join(format!("src/uu/checksum_common/locales/{locale}.ftl"))
		})?;
	}
...
fn embed_static_utility_locales(
	...
	writeln!(embedded_file, "        // Static utility locales for crates.io builds")?;
	...
	// First, try to embed uucore locales - critical for common translations like
	// "Usage:"
	embed_component_locales(embedded_file, locales_to_embed, "uucore", |locale| {
...
///
/// To support localized messages in installed binaries (e.g., via `cargo
/// install`), this function identifies the user's current locale from the
...
fn embed_locale_file(
	...
	if locale_path.exists() || locale_path.is_file() {
		...
		writeln!(embedded_file, "        // Locale for {component} ({locale})")?;
...
/// Higher-level helper to embed locale files for a component with a path
/// pattern.
///
/// This eliminates the repetitive `for_each_locale` + `embed_locale_file`
/// pattern.
///
/// # Errors
///
