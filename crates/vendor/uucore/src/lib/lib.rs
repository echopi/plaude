...
pub use crate::mods::{display, error, line_ending, locale, os, panic, posix};

/// Disables the custom signal handlers installed by Rust for stack-overflow
/// handling. With those custom signal handlers processes ignore the first
/// SIGBUS and SIGSEGV signal they receive. See <https://github.com/rust-lang/rust/blob/8ac1525e091d3db28e67adcbbd6db1e1deaa37fb/src/libstd/sys/unix/stack_overflow.rs#L71-L92> for details.
#[cfg(unix)]
pub fn disable_rust_signal_handlers() -> Result<(), Errno> {
	unsafe { sigaction(SIGSEGV, &SigAction::new(SigDfl, SaFlags::empty(), SigSet::all())) }?;
	unsafe { sigaction(SIGBUS, &SigAction::new(SigDfl, SaFlags::empty(), SigSet::all())) }?;
	Ok(())
}
...
///
/// This macro expands to a main function that invokes the `uumain` function in
/// `util` Exits with code returned by `uumain`.
#[macro_export]
macro_rules! bin {
	($util:ident) => {
		pub fn main() {
...
/// example: "(uutils coreutils) 0.30.0". clap will then prefix it with the util
/// name.
#[macro_export]
...
///
/// let app = Command::new("myutil").help_template(localized_help_template("myutil"));
/// ```
pub fn localized_help_template(util_name: &str) -> clap::builder::StyledStr {
	use std::io::IsTerminal;

	// Determine if colors should be enabled - same logic as
...
pub fn localized_help_template_with_colors(
	...
	if colors_enabled {
		write!(template, "\x1b[1m\x1b[4m{usage_label}:\x1b[0m {{usage}}\n\n").unwrap();
	} else {
...
/// Used to check if we were called as a multicall binary (`coreutils
/// <utility>`)
...
const FALLBACK_UTILITY_NAME: &str = "coreutils";

fn derive_utility_name(argv: &[OsString], utility_is_second_arg: bool) -> String {
	let base_index = usize::from(utility_is_second_arg);
	let is_man = usize::from(argv.get(base_index).is_some_and(|arg| arg == "manpage"));
	let argv_index = base_index + is_man;
	let utility_arg = argv
		.get(argv_index)
		.or_else(|| argv.get(base_index))
		.or_else(|| argv.first());
	...
	utility_arg.map_or_else(
		|| FALLBACK_UTILITY_NAME.to_owned(),
		|arg| {
...
fn derive_execution_phrase(argv: &[OsString], utility_is_second_arg: bool) -> String {
	if utility_is_second_arg {
		let phrase = argv
			.iter()
			.take(2)
		...
		argv
			.first()
			.map_or_else(|| FALLBACK_UTILITY_NAME.to_owned(), |arg| arg.to_string_lossy().into_owned())
	}
}

static UTIL_NAME: LazyLock<String> =
	LazyLock::new(|| derive_utility_name(&ARGV, get_utility_is_second_arg()));
...
static EXECUTION_PHRASE: LazyLock<String> =
	LazyLock::new(|| derive_execution_phrase(&ARGV, get_utility_is_second_arg()));
...
pub trait Args: Iterator<Item = OsString> + Sized {
	/// Collects the iterator into a `Vec<String>`, lossily converting the
	/// `OsString`s to `Strings`.
	fn collect_lossy(self) -> Vec<String> {
		self.map(|s| s.to_string_lossy().into_owned()).collect()
	}

	/// Collects the iterator into a `Vec<String>`, removing any elements that
	/// contain invalid encoding.
...
/// Returns an iterator over the command line arguments as `OsString`s,
/// filtering out empty arguments. This is useful for handling cases where extra
/// whitespace or empty arguments are present. args_os_filtered() can be
/// expensive to call
pub fn args_os_filtered() -> impl Iterator<Item = OsString> {
	ARGV.iter().filter(|arg| !arg.is_empty()).cloned()
}

/// Read a line from stdin and check whether the first character is `'y'` or
/// `'Y'`
...
impl std::fmt::Display for NonUtf8OsStrError {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		f.write_fmt(format_args!(
			"invalid UTF-8 input {quoted} encountered when converting to bytes on a platform that \
...
pub fn os_str_from_bytes(bytes: &[u8]) -> error::UResult<Cow<'_, OsStr>> {
	...
	#[cfg(not(any(unix, target_os = "wasi")))]
	Ok(Cow::Owned(OsString::from(
		str::from_utf8(bytes)
			.map_err(|_| error::UUsageError::new(1, "Unable to transform bytes into OsStr"))?,
	)))
}

/// Converts a `Vec<u8>` into an `OsString`, parsing as UTF-8 on non-unix
/// platforms.
///
/// This always succeeds on unix platforms,
/// and fails on other platforms if the bytes can't be parsed as UTF-8.
...
/// Converts an `OsString` into a `Vec<u8>`, parsing as UTF-8 on non-unix
/// platforms.
///
/// This always succeeds on unix platforms,
/// and fails on other platforms if the bytes can't be parsed as UTF-8.
...
/// Prompt the user with a formatted string and returns `true` if they reply
/// `'y'` or `'Y'`
///
/// This macro functions accepts the same syntax as `format!`. The prompt is
/// written to `stderr`. A space is also printed at the end for nice spacing
/// between the prompt and the user input. Any input starting with `'y'` or
/// `'Y'` is interpreted as `yes`.
///
/// # Examples
/// ```
...
/// ```
/// will print something like below to `stderr` (with `util_name` substituted by
/// the actual util name) and will wait for user input.
/// ```txt
...
impl<'a> CharByteIterator<'a> {
	...
	pub fn new(input: &'a [u8]) -> Self {
		Self { iter: Box::new(input.utf8_chunks().flat_map(Utf8ChunkIterator::from)) }
	}
}
...
mod tests {
	...
	fn test_invalid_utf8_args_lossy(os_str: &OsStr) {
		...
		// conservation of length - when accepting lossy conversion no arguments may be
		// dropped
		...
		assert_eq!(*collected_to_str[2], os_str.to_os_string().to_string_lossy());
	}
	...
	fn valid_utf8_encoding_args() {
		...
		// expect complete conversion without losses, even when lossy conversion is
		// accepted
	...
	#[test]
	fn utility_name_falls_back_when_argv_is_empty() {
		let argv = [];

		assert_eq!(derive_utility_name(&argv, false), FALLBACK_UTILITY_NAME);
		assert_eq!(derive_execution_phrase(&argv, false), FALLBACK_UTILITY_NAME);
	}

	#[test]
	fn utility_name_falls_back_when_second_arg_is_missing() {
		let argv = [OsString::from("/tmp/coreutils")];

		assert_eq!(derive_utility_name(&argv, true), "coreutils");
		assert_eq!(derive_execution_phrase(&argv, true), "/tmp/coreutils");
	}
