...
impl From<std::io::Error> for LocalizationError {
	fn from(error: std::io::Error) -> Self {
		Self::Io { source: error, path: PathBuf::from("<unknown>") }
	}
}
...
impl Localizer {
	fn new(primary_bundle: FluentBundle<&'static FluentResource>) -> Self {
		Self { primary_bundle, fallback_bundle: None }
	}
...
/// Helper function to find the uucore locales directory from a utility's
/// locales directory
...
fn create_bundle(
	...
	let mut try_add_resource_from = |dir_opt: Option<PathBuf>| {
		...
		{
			// use Box::leak to provide 'static lifetime for shared FluentBundle between
			// threads
	...
	// Then, try to load utility-specific strings from the utility's locale
	// directory
	...
	if ["cksum", "b2sum", "md5sum", "sha1sum", "sha224sum", "sha256sum", "sha384sum", "sha512sum"]
		.contains(&util_name)
	{
...
/// Initialize localization with common strings in addition to utility-specific
/// strings
		...
		|(_partial_resource, errs): (FluentResource, Vec<ParserError>)| {
			if let Some(first_err) = errs.into_iter().next() {
				let snippet = first_err
					...
					.unwrap_or("")
					.to_string();
				LocalizationError::ParseResource { error: first_err, snippet }
			} else {
...
/// Create a bundle from embedded English locale files with common uucore
/// strings
fn create_english_bundle_from_embedded(
	...
	// Return the bundle if we have either common strings or utility-specific
	// strings
	if bundle.has_message("common-error") || bundle.has_message(&format!("{util_name}-about")) {
		Ok(bundle)
	} else {
...
///
/// Looks up a message with the given ID in the current locale bundle and
/// returns the localized text. If the message ID is not found in the current
/// locale, it will fall back to English. If the message is not found in English
/// either, returns the message ID itself.
///
/// # Arguments
///
...
/// # Returns
///
/// A `String` containing the localized message with variable substitution, or
/// the message ID if not found
///
/// # Examples
///
/// ```
...
fn detect_system_locale() -> Result<LanguageIdentifier, LocalizationError> {
	...
	LanguageIdentifier::from_str(&locale_str)
		.map_err(|_| LocalizationError::ParseLocale(format!("Failed to parse locale: {locale_str}")))
}
...
///
/// This function initializes the localization system based on the system's
/// locale preferences (via the LANG environment variable) or falls back to
/// English if the system locale cannot be determined or the locale file doesn't
/// exist. English is always loaded as a fallback.
///
/// # Arguments
///
...
/// Helper function to get the locales directory based on the build
/// configuration
...
mod tests {
	...
	fn create_test_bundle(
		...
		// Only load from the test directory - no common strings or utility-specific
		// paths
...
";
		...
		fs::write(temp_dir.path().join("en-US.ftl"), en_content).expect("Failed to write en-US.ftl");
		fs::write(temp_dir.path().join("fr-FR.ftl"), fr_content).expect("Failed to write fr-FR.ftl");
		fs::write(temp_dir.path().join("ja-JP.ftl"), ja_content).expect("Failed to write ja-JP.ftl");
		fs::write(temp_dir.path().join("ar-SA.ftl"), ar_content).expect("Failed to write ar-SA.ftl");
	...
	fn test_create_bundle_invalid_syntax() {
		...
		match result {
			Err(LocalizationError::ParseResource { error: _parser_err, snippet: _ }) => {
	...
	fn test_localizer_format_primary_bundle() {
		let temp_dir = create_test_locales_dir();
		let en_bundle: FluentBundle<&'static FluentResource> =
			create_test_bundle(&LanguageIdentifier::from_str("en-US").unwrap(), temp_dir.path())
				.unwrap();
	...
	fn test_localizer_fallback_to_english() {
		let temp_dir = create_test_locales_dir();
		let fr_bundle =
			create_test_bundle(&LanguageIdentifier::from_str("fr-FR").unwrap(), temp_dir.path())
				.unwrap();
		let en_bundle =
			create_test_bundle(&LanguageIdentifier::from_str("en-US").unwrap(), temp_dir.path())
				.unwrap();
	...
	fn test_parse_resource_error_includes_snippet() {
		...
		if let Err(LocalizationError::ParseResource { error: _err, snippet }) = result {
	...
	fn test_detect_system_locale_from_lang_env() {
		// Test locale parsing logic directly instead of relying on environment
		// variables which can have race conditions in multi-threaded test
		// environments
	...
	fn test_setup_localization_success() {
		std::thread::spawn(|| {
			...
			// Test that we can get messages (should use embedded English for "test"
			// utility)
