...
//!
//! This module provides utilities for handling clap errors with localization
//! support. It uses clap's error context API to extract structured information
//! from errors instead of parsing error strings, providing a more robust
//! solution.
...
impl<'a> ErrorFormatter<'a> {
	pub fn new(util_name: &'a str) -> Self {
		Self { color_mgr: ColorManager::from_env(), util_name }
	}
	...
	fn handle_invalid_value(&self, err: &Error, exit_code: i32) -> i32 {
		...
		// This allows utilities like runcon (125) to override the default while
		// preserving the standard behavior for utilities using normal error codes (1,
		// 2, etc.)
	...
	fn print_simple_error_msg(&self, message: &str) {
		let error_word = translate!("common-error");
		let _ = writeln!(stderr(), "{}: {message}", self.color_mgr.colorize(&error_word, Color::Red));
	}
...
///
/// This is the main entry point for processing command-line arguments with
/// localized error messages. It parses the provided arguments and returns
/// either the parsed matches or handles errors with localized messages.
///
/// # Arguments
///
...
/// when encountering parsing errors (except help/version which use exit code
/// 0).
///
/// # Examples
///
...
///
/// This function processes a clap error and exits the program with the
/// specified exit code. It formats error messages with proper localization and
/// color support based on environment variables.
///
/// # Arguments
///
...
/// - Appropriate color settings based on environment variables (`NO_COLOR`,
///   `CLICOLOR_FORCE`, etc.)
...
/// - `CLICOLOR_FORCE` or `FORCE_COLOR` - Forces color output even when not in a
///   TTY
...
///
/// let cmd = Command::new("myutil").arg(clap::Arg::new("input").short('i'));
