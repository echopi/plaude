...
//!
//! For writing raw paths to stdout when the output should not be quoted or
//! escaped, use `println_verbatim`. This will preserve invalid unicode.
//!
//! # Examples
//! ```rust
//! use std::path::Path;
...
use std::{
	...
	fmt,
	fs::File,
	io::{self, BufWriter, Stdout, StdoutLock, Write as IoWrite},
};

// These used to be defined here, but they live in their own crate now.
pub use os_display::{Quotable, Quoted};

/// Print a path (or `OsStr`-like object) directly to stdout, with a trailing
/// newline, without losing any information if its encoding is invalid.
///
/// This function is appropriate for commands where printing paths is the point
/// and the output is likely to be captured, like `pwd` and `basename`. For
/// informational output use `Quotable::quote`.
///
/// FIXME: Invalid Unicode will produce an error on Windows. That could be fixed
/// by using low-level library calls and bypassing `io::Write`. This is not a
/// big priority because broken filenames are much rarer on Windows than on
/// Unix.
...
///
/// On Windows this currently returns an error if the OS string is not valid
/// Unicode. This may in the future change to allow those strings to be written
/// to consoles.
pub trait OsWrite: io::Write {
	...
	fn write_all_os(&mut self, buf: &OsStr) -> io::Result<()> {
		...
		{
			// It's possible to write a better OsWrite impl for Windows consoles (e.g.
			// Stdout) as those are fundamentally 16-bit. If the OS string is invalid
			// then it can be encoded to 16-bit and written using raw windows_sys calls.
			// But this is quite involved (see `sys/pal/windows/stdio.rs` in the stdlib)
			// and the value-add is small.
			//
			// There's no way to write invalid OS strings to Windows files, as those are
			// 8-bit.
...
// We do not have a blanket impl for all Write because a smarter Windows impl
// should be able to make use of AsRawHandle. Please keep this in mind when
// adding new impls.
...
/// Print all environment variables in the format `name=value` with the
/// specified line ending.
///
/// This function handles non-UTF-8 environment variable names and values
/// correctly by using raw bytes on Unix systems.
