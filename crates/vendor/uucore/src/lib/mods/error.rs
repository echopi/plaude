...
//! All utils return exit with an exit code. Usually, the following scheme is
//! used:
//! * `0`: succeeded
//! * `1`: minor problems
//! * `2`: major problems
//!
//! This module provides types to reconcile these exit codes with idiomatic Rust
//! error handling. This has a couple advantages over manually using
//! [`std::process::exit`]:
//! 1. It enables the use of `?`, `map_err`, `unwrap_or`, etc. in `uumain`.
//! 1. It encourages the use of [`UResult`]/[`Result`] in functions in the
//!    utils.
...
//! 1. [`set_exit_code`] takes away the burden of manually tracking exit codes
...
//! }
//! ```
//! [`UResult`] is a simple wrapper around [`Result`] with a custom error trait:
//! [`UError`]. The most important difference with types implementing
//! [`std::error::Error`] is that [`UError`]s can specify the exit code of the
//! program when they are returned from `uumain`:
//! * When `Ok` is returned, the code set with [`set_exit_code`] is used as exit
//!   code. If [`set_exit_code`] was not used, then `0` is used.
//! * When `Err` is returned, the code corresponding with the error is used as
//!   exit code and the error message is displayed.
//!
//! Additionally, the errors can be displayed manually with the [`crate::show`]
//! and [`crate::show_if_err`] macros: 
//! ```ignore
...
//! ```
//!
//! **Note**: The [`crate::show`] and [`crate::show_if_err`] macros set the exit
//! code of the program using [`set_exit_code`]. See the documentation on that
...
//! * Using [`ExitCode`] is not recommended but can be useful for converting
//!   utils to use [`UResult`].
...
///
/// This function is most useful for non-fatal errors, for example when applying
/// an operation to multiple files:
/// ```ignore
...
/// [`Box<dyn UError>`]. The implementation for `From` is provided
/// automatically.
///
...
/// A simple error type with an exit code and a message that implements
/// [`UError`].
///
/// ```
...
/// Wrapper type around [`std::io::Error`].
///
/// The messages displayed by [`UIoError`] should match the error messages
/// displayed by GNU coreutils.
///
/// There are two ways to construct this type: with [`UIoError::new`] or by
/// calling the [`FromIo::map_err_context`] method on a [`std::io::Result`] or
/// [`std::io::Error`]. 
/// ```
...
/// use uucore::{
/// 	display::Quotable,
/// 	error::{FromIo, UError, UIoError, UResult},
/// };
/// let path = Path::new("test.txt");
///
/// // Manual construction
/// let e: Box<dyn UError> =
/// 	UIoError::new(std::io::ErrorKind::NotFound, format!("cannot access {}", path.quote()));
/// let res: UResult<()> = Err(e.into());
///
/// // Converting from an `std::io::Error`.
/// let res: UResult<File> =
/// 	File::open(path).map_err_context(|| format!("cannot access {}", path.quote()));
/// ```
#[derive(Debug)]
pub struct UIoError {
...
impl UIoError {
	...
	pub fn new<S: Into<String>>(kind: std::io::ErrorKind, context: S) -> Box<dyn UError> {
		Box::new(Self { context: Some(context.into()), inner: kind.into() })
	}
}
...
/// Enables the conversion from [`std::io::Error`] to [`UError`] and from
/// [`std::io::Result`] to [`UResult`].
pub trait FromIo<T> {
	/// Map the error context of an [`std::io::Error`] or [`std::io::Result`] to
	/// a custom error
...
impl FromIo<Box<UIoError>> for std::io::Error {
	fn map_err_context(self, context: impl FnOnce() -> String) -> Box<UIoError> {
		Box::new(UIoError { context: Some(context()), inner: self })
	}
}
...
impl FromIo<Box<UIoError>> for std::io::ErrorKind {
	fn map_err_context(self, context: impl FnOnce() -> String) -> Box<UIoError> {
		Box::new(UIoError { context: Some(context()), inner: std::io::Error::new(self, "") })
	}
}

impl From<std::io::Error> for UIoError {
	fn from(f: std::io::Error) -> Self {
		Self { context: None, inner: f }
	}
}
...
impl From<nix::Error> for UIoError {
	fn from(f: nix::Error) -> Self {
		Self { context: None, inner: std::io::Error::from_raw_os_error(f as i32) }
	}
}
...
///
/// ```
/// use uucore::{error::UIoError, uio_error};
///
/// let io_err = std::io::Error::new(std::io::ErrorKind::PermissionDenied, "fix me please!");
///
/// let uio_err = UIoError::new(io_err.kind(), format!("Error code: {}", 2));
///
...
/// ```
/// This type is especially useful for a trivial conversion from utils returning
/// [`i32`] to returning [`UResult`].
#[derive(Debug)]
...
/// [`UClapError::with_exit_code`] method on [`clap::Error`] or using the
/// [`From`] implementation from [`clap::Error`] to `Box<dyn UError>`, which
/// constructs a [`ClapErrorWrapper`] with an exit code of `1`.
///
/// ```rust
/// use uucore::error::{ClapErrorWrapper, UClapError, UError};
...
impl UClapError<ClapErrorWrapper> for clap::Error {
	fn with_exit_code(self, code: i32) -> ClapErrorWrapper {
		ClapErrorWrapper { code, error: self, print_failed: Cell::new(false) }
	}
}
