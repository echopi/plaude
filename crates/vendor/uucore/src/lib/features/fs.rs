...
use std::{
	...
	fs::read_dir,
	hash::Hash,
	io::{Error, ErrorKind, Result as IOResult, Stdin},
...
impl FileInformation {
	...
	///
	/// If `path` points to a symlink and `dereference` is true, information
	/// about the link's target will be returned.
...
///   [`std::fs::canonicalize`], resolving symbolic links and returning an error
///   if the path does not exist.
/// * [`MissingHandling::Missing`] makes this function ignore non-final
///   components of the path that could not be resolved.
/// * [`MissingHandling::Existing`] makes this function return an error if the
///   final component of the path does not exist.
///
/// The `res_mode` parameter controls how symbolic links are
/// resolved:
///
/// * [`ResolveMode::None`] makes this function not try to resolve any symbolic
///   links.
/// * [`ResolveMode::Physical`] makes this function resolve symlinks as they are
///   encountered
		...
		match resolve_symlink(&result) {
			Ok(Some(link_path)) => {
				...
				if followed_symlinks < SYMLINKS_TO_LOOK_FOR_LOOPS {
					...
					let file_info = FileInformation::from_path(result.parent().unwrap(), false).unwrap();
...
///
/// A symlink loop is a chain of symlinks where the last symlink points back to
/// one of the previous symlinks in the chain.
///
/// # Arguments
///
/// * `path` - A reference to a `Path` representing the starting path to check
///   for symlink loops.
///
/// # Returns
///
...
pub fn is_symlink_loop(path: &Path) -> bool {
	...
	while let (Ok(metadata), Ok(link)) =
		(current_path.symlink_metadata(), fs::read_link(&current_path))
	{
...
/// # Returns
///
/// * `bool` - Returns `true` if the paths are hard links to the same file, and
///   `false` otherwise.
#[cfg(unix)]
...
/// Checks if either two paths are hard links to the same file or if the source
/// path is a symbolic link which when fully resolved points to target path
///
/// # Arguments
///
/// * `source` - A reference to a `Path` representing the source path.
...
/// # Returns
///
/// * `bool` - Returns `true` if either of above conditions are true, and
///   `false` otherwise.
#[cfg(unix)]
...
/// Extracts the filename component from the given `file` path and returns it as
/// an `Option<&str>`.
///
/// If the `file` path contains a filename, this function returns
/// `Some(filename)` where `filename` is the extracted filename as a string
/// slice (`&str`). If the `file` path does not have a filename component or if
/// the filename is not valid UTF-8, it returns `None`.
///
/// # Arguments
///
/// * `file`: A reference to a `Path` representing the file path from which to
///   extract the filename.
///
/// # Returns
///
/// * `Some(filename)`: If a valid filename exists in the `file` path, where
///   `filename` is the extracted filename as a string slice (`&str`).
/// * `None`: If the `file` path does not contain a valid filename or if the
...
mod tests {
	...
	const NORMALIZE_PATH_TESTS: [NormalizePathTestCase; 15] = [
		NormalizePathTestCase { path: "foo/bar/../..", test: "." },
		NormalizePathTestCase { path: ".", test: "." },
		// Should not try to eliminate leading .. components,
		// as it may point to a sibling of the current dir
		NormalizePathTestCase { path: "../foo", test: "../foo" },
		// Try to go down, then escape above current dir and back down again
		NormalizePathTestCase { path: "foo/../../../bar/baz", test: "../../bar/baz" },
		NormalizePathTestCase { path: "../../foo/..", test: "../.." },
		NormalizePathTestCase { path: "foo/../../..", test: "../.." },
		NormalizePathTestCase { path: "foo/bar/../../..", test: ".." },
		NormalizePathTestCase { path: "./foo/bar.txt", test: "foo/bar.txt" },
		NormalizePathTestCase { path: "bar/../foo/bar.txt", test: "foo/bar.txt" },
		NormalizePathTestCase { path: "foo///bar.txt", test: "foo/bar.txt" },
		NormalizePathTestCase { path: "foo///bar", test: "foo/bar" },
		NormalizePathTestCase { path: "foo//./bar", test: "foo/bar" },
		NormalizePathTestCase { path: "/foo//./bar", test: "/foo/bar" },
		NormalizePathTestCase { path: r"C:/you/later/", test: "C:/you/later" },
