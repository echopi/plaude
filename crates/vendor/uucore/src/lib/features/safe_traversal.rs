...
// This module provides TOCTOU-safe filesystem operations for recursive
// traversal
//
// Available on Unix
//
// spell-checker:ignore CLOEXEC RDONLY TOCTOU closedir dirp fdopendir fstatat
// openat REMOVEDIR unlinkat smallfile spell-checker:ignore RAII dirfd fchownat
// fchown FchmodatFlags fchmodat fchmod mkdirat CREAT WRONLY ELOOP ENOTDIR
...
use nix::{
	...
	fcntl::{OFlag, openat},
	libc,
	sys::stat::{FchmodatFlags, FileStat, Mode, fchmodat, fstatat, mkdirat},
	unistd::{Gid, Uid, UnlinkatFlags, fchown, fchownat, unlinkat},
};
...
/// at call sites. Instead of `open(path, true)`, use `open(path,
...
impl DirFd {
	...
	pub fn fchmod(&self, mode: u32) -> io::Result<()> {
		...
		nix::sys::stat::fchmod(&self.fd, mode).map_err(|e| io::Error::from_raw_os_error(e as i32))?;
	...
	pub fn mkdir_at(&self, name: &OsStr, mode: u32) -> io::Result<()> {
		...
		if let Err(e) = mkdirat(self.fd.as_fd(), name_cstr.as_c_str(), mode) {
			...
			return Err(SafeTraversalError::OpenFailed { path: name.into(), source: err }.into());
		}
		Ok(())
	}
...
///
/// Returns the existing ancestor path and a list of components that need to be
/// created. Uses `symlink_metadata` to detect symlinks - symlinks are NOT
/// followed and are treated as components that need to be created/replaced.
...
/// Safely create all parent directories for a path using directory file
/// descriptors. This prevents symlink race conditions by anchoring all
/// operations to directory fds.
///
/// # Security
/// This function prevents TOCTOU race conditions by:
/// 1. Finding the deepest existing ancestor directory (path-based, but safe
///    since it exists)
/// 2. Opening that ancestor with a file descriptor
/// 3. Creating all new directories using fd-based operations (mkdirat, openat
///    with O_NOFOLLOW)
///
/// Once we have a fd for an existing ancestor, all subsequent operations use
/// that fd as the anchor. If an attacker replaces a newly-created directory
/// with a symlink, our openat with O_NOFOLLOW will fail, preventing the attack.
///
/// Existing symlinks in the path (like /var -> /private/var on macOS) are
/// followed when finding the ancestor, which is safe since they already exist.
///
/// # Arguments
/// * `path` - The path to create directories for
/// * `mode` - The mode to use when creating new directories (e.g., 0o755). The
///   actual mode will be modified by the process umask.
///
/// # Returns
...
impl FileInfo {
	pub fn from_stat(stat: &libc::stat) -> Self {
		// Allow unnecessary cast because st_dev and st_ino have different types on
		// different platforms
		#[allow(clippy::unnecessary_cast)]
		Self { dev: stat.st_dev as u64, ino: stat.st_ino as u64 }
	}
...
mod tests {
	use std::{
		fs,
		os::unix::{fs::symlink, io::IntoRawFd},
	};
	...
	#[test]
	fn test_error_chain() {
		let result = DirFd::open("/nonexistent/deeply/nested/path".as_ref(), SymlinkBehavior::Follow);
		assert!(result.is_err());
