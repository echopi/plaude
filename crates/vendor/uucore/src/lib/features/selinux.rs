...
impl UError for SeLinuxError {
	fn code(&self) -> i32 {
		match self {
			...
			Self::ContextSetFailure(..) => 4,
			Self::ContextConversionFailure(..) => 5,
...
/// Note: libselinux internally caches this value, so no additional caching is
/// needed.
...
///
/// If a specific context is provided, it attempts to set this context
/// explicitly. Otherwise, it applies the default SELinux context for the
/// provided path.
///
/// # Arguments
///
...
pub fn set_selinux_security_context(
	...
	if let Some(ctx_str) = context {
		...
		let c_context = std::ffi::CString::new(ctx_str.as_str()).map_err(|e| {
			SeLinuxError::ContextConversionFailure(ctx_str.to_owned(), selinux_error_description(&e))
		})?;
...
///
/// Retrieves the security context of the specified filesystem path if SELinux
...
/// # Returns
///
/// * `Ok(String)` - The SELinux context string if successfully retrieved.
///   Returns an empty string if no context was found.
/// * `Err(SeLinuxError)` - An error variant indicating the type of failure:
///   - `SeLinuxError::SELinuxNotEnabled` - SELinux is not enabled on the
///     system.
///   - `SeLinuxError::FileOpenFailure` - Failed to open the specified file.
///   - `SeLinuxError::ContextRetrievalFailure` - Failed to retrieve the
///     security context.
///   - `SeLinuxError::ContextConversionFailure` - Failed to convert the
///     security context to a string.
...
/// use std::path::Path;
///
/// use uucore::selinux::{SeLinuxError, get_selinux_security_context};
///
...
/// 		println!("Failed to retrieve the security context: {e}")
/// 	},
/// 	Err(SeLinuxError::ContextConversionFailure(ctx, e)) => {
/// 		println!("Failed to convert context '{ctx}': {e}")
/// 	},
/// 	Err(SeLinuxError::ContextSetFailure(ctx, e)) => {
/// 		println!("Failed to set context '{ctx}': {e}")
/// 	},
...
pub fn get_selinux_security_context(
	...
	let context = match SecurityContext::of_path(path, follow_symbolic_links, false) {
		...
		Err(e) => {
			return Err(SeLinuxError::ContextRetrievalFailure(selinux_error_description(&e)));
		},
	};
...
///
/// This function retrieves and compares the SELinux security contexts of two
/// paths. If the contexts differ or an error occurs during retrieval, it
/// returns true.
///
/// # Arguments
///
...
/// # Returns
///
/// * `true` - If contexts differ, cannot be retrieved, or if SELinux is not
///   enabled.
...
///
/// This function copies the security context from the source path to the
/// destination path. If SELinux is not enabled, or if the source has no
/// context, the function returns success without making any changes.
///
/// # Arguments
///
/// * `from_path` - Source filesystem path from which to copy the SELinux
///   context.
/// * `to_path` - Destination filesystem path to which the context should be
///   applied.
///
/// # Returns
///
/// * `Ok(())` - If the context was successfully preserved or if SELinux is not
///   enabled.
/// * `Err(SeLinuxError)` - If an error occurred during context retrieval or
///   application.
///
/// # Examples
///
...
mod tests {
	...
	fn test_selinux_context_setting() {
		...
		assert!(default_result.is_ok(), "Failed to set default context: {:?}", default_result.err());

		let context = get_selinux_security_context(path, false).expect("Failed to get context");
		assert!(!context.is_empty(), "Expected non-empty context after setting default context");
		...
		if explicit_result.is_ok() {
			...
			println!("Note: Could not set explicit context {:?}", explicit_result.err());
		}
	}
	#[test]
	...
	fn test_get_selinux_context_symlink() {
		...
		let file_context =
			get_selinux_security_context(&file_path, false).expect("Failed to get security context.");
	...
	fn test_contexts_differ() {
		...
		if result1.is_ok() && result2.is_ok() {
			...
			println!(
				"Note: Couldn't set SELinux contexts to test differences. This is expected if the \
				 test doesn't have sufficient permissions."
			);
			assert!(
				contexts_differ(path1, path2),
				"Contexts should differ when different contexts are set"
	...
	fn test_preserve_security_context() {
		...
		if result.is_ok() {
			...
			println!(
				"Note: Couldn't set SELinux context on source file to test preservation. This is \
				 expected if the test doesn't have sufficient permissions."
			);
