...
impl SizedAlgoKind {
	...
	pub fn to_tag(self) -> String {
		match self {
			...
			Self::Shake128(opt_bit_len) => {
				format!("SHAKE128-{}", opt_bit_len.unwrap_or(Shake128::DEFAULT_BIT_SIZE))
			},
			Self::Shake256(opt_bit_len) => {
				format!("SHAKE256-{}", opt_bit_len.unwrap_or(Shake256::DEFAULT_BIT_SIZE))
			},
