...
use std::{
	...
	fmt::Display,
	fs::File,
	io::{self, BufReader, Read, Write, stderr, stdin},
};
...
use crate::{
	...
	read_os_string_lines, show, show_warning_caps,
...
fn print_cksum_report(res: &ChecksumResult) {
	if res.bad_format > 0 {
		show_warning_caps!("{}", translate!("checksum-bad-format", "count" => res.bad_format));
	}

	if res.failed_cksum > 0 {
		show_warning_caps!("{}", translate!("checksum-failed-cksum", "count" => res.failed_cksum));
	}
