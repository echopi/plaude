...
// spell-checker:ignore (vars) NANOS numstr infinityh INFD nans nanh bigdecimal
// extendedbigdecimal
...
mod tests {
	...
	fn test_underflow() {
		...
		assert_eq!(from_str("1e-92233720368547758080", true), Ok(NANOSECOND_DURATION));
		...
		// ExtendedBigDecimal underflow
		assert_eq!(from_str("1e-92233720368547758080", false), Ok(NANOSECOND_DURATION));
		assert_eq!(from_str("0x6p-4376646810043701", false), Ok(NANOSECOND_DURATION));
	...
	fn test_zero() {
		...
		assert_eq!(from_str("0e-92233720368547758080", true), Ok(Duration::ZERO));
		assert_eq!(from_str("0.000000000000000000000", true), Ok(Duration::ZERO));

		assert_eq!(from_str("0e-9", false), Ok(Duration::ZERO));
		assert_eq!(from_str("0e-100", false), Ok(Duration::ZERO));
		assert_eq!(from_str("0e-92233720368547758080", false), Ok(Duration::ZERO));
		assert_eq!(from_str("0.000000000000000000000", false), Ok(Duration::ZERO));
	}

	#[test]
	fn test_hex_float() {
		assert_eq!(from_str("0x1.1p-1", true), Ok(Duration::from_secs_f64(0.53125f64)));
		assert_eq!(from_str("0x1.1p-1", false), Ok(Duration::from_secs_f64(0.53125f64)));
		assert_eq!(
