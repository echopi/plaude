...
/// `ShortcutValueParser` is similar to clap's `PossibleValuesParser`: it
/// verifies that the value is from an enumerated set of `PossibleValue`.
///
/// Whereas `PossibleValuesParser` only accepts exact matches,
/// `ShortcutValueParser` also accepts shortcuts as long as they are
/// unambiguous.
impl ShortcutValueParser {
	...
	fn generate_clap_error(
		...
		// if `possible_values` is not empty then that means this error is because of an
		// ambiguous value.
...
/// Adds a suggestion when error is because of ambiguous values based on the
...
fn add_ambiguous_value_tip(
	...
	err.insert(
		...
		ContextValue::StyledStrs(vec![
			format!(
				"It looks like '{value}' could match several values. Did you mean \
...
mod tests {
	...
	fn test_parse_ref_with_ambiguous_value() {
		...
		for ambiguous_value in ambiguous_values {
			...
			assert!(result.unwrap_err().to_string().contains(&format!(
				"It looks like '{ambiguous_value}' could match several values. Did you mean 'abcd' or \
				 'abef'?"
			)));
		}
	...
	fn test_ambiguous_word_same_meaning() {
		let cmd = Command::new("cmd");
		let parser =
			ShortcutValueParser::new([PossibleValue::new("atime").alias("access"), "status".into()]);
