...
//! Parser for signed numeric arguments used by head, tail, and similar
//! utilities.
//!
//! These utilities accept arguments like `-5`, `+10`, `-100K` where the leading
//! sign indicates different behavior (e.g., "first N" vs "last N" vs "starting
//! from N").
...
///
/// Note: On parse failure, this returns an error with the raw string (without
/// quotes) to allow callers to format the error message as needed.
...
/// Strip the sign prefix from a string and return both the sign and remaining
/// string.
