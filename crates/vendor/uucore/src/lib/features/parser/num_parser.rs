...
// spell-checker:ignore powf copysign prec ilog inity infinit infs bigdecimal
// extendedbigdecimal biguint underflowed muls
...
impl Base {
	...
	/// Greedily parse as many digits as possible from the string, adding to
	/// already parsed digits. This is meant to be used (directly) for the part
	/// after a decimal point. Returns parsed digits (if any), the number of
	/// parsed digits, and the rest of the string.
	fn parse_digits_count(self, str: &str, digits: Option<BigUint>) -> (Option<BigUint>, i64, &str) {
		...
		while let Some(d) = rest.chars().next().and_then(|c| self.digit(c)) {
			(digits_tmp, count_tmp, mul_tmp) =
				(digits_tmp * self as u64 + d, count_tmp + 1, mul_tmp * self as u64);
			rest = &rest[1..];
			// In base 16, we parse 4 bits at a time, so we can parse 16 digits at most in a
			// u64.
			if count_tmp >= 15 {
				// Accumulate what we have so far
				(digits, count) =
					(Some(digits.unwrap_or_default() * mul_tmp + digits_tmp), count + count_tmp);
				// Reset state
	...
	T: Zero,
{
	...
	/// conversion.
	fn map<U>(self, f: impl FnOnce(T) -> Result<U, ExtendedParserError<U>>) -> ExtendedParserError<U>
	where
		U: Zero,
	{
...
/// A number parser for binary, octal, decimal, hexadecimal and single
/// characters.
///
...
pub trait ExtendedParser {
	// We pick a hopefully different name for our parser, to avoid clash with
	// standard traits.
...
impl ExtendedParser for f64 {
	...
	fn extended_parse(input: &str) -> Result<Self, ExtendedParserError<Self>> {
		fn into_f64(ebd: ExtendedBigDecimal) -> Result<f64, ExtendedParserError<f64>> {
			// TODO: _Some_ of this is generic, so this should probably be implemented as an
			// ExtendedBigDecimal trait (ToPrimitive).
			let v = match ebd {
				ExtendedBigDecimal::BigDecimal(bd) => {
...
fn parse_digits(base: Base, str: &str, fractional: bool) -> (Option<BigUint>, i64, &str) {
	...
	// If allowed, parse the fractional part of the number if there can be one and
	// the input contains a '.' decimal separator.
	if fractional {
...
fn parse_special_value(
	...
	// Array of ("String to match", return value when sign positive, when sign
	// negative)
	...
	for (str, ebd) in MATCH_TABLE {
		if input_lc.starts_with(str) {
			...
			// "infs" is a valid duration, so parse suffix multiplier in the original input
			// string, but ignore the multiplier.
...
fn construct_extended_big_decimal(
	...
	let bd = if scale == 0 && exponent.is_zero() {
		...
		if exponent.is_zero() {
			...
			// Note that new_scale is a negative exponent: large positive value causes an
			// underflow, large negative values an overflow.
		...
		// pow "only" supports u32 values, just error out if given more than 2**32
		// fractional digits.
		...
		// powi "only" supports i64 values. Just overflow/underflow if the value
		// provided is > 2**64 or < 2**-64.
		...
		// Note: We cannot overflow/underflow BigDecimal here, as we will not be able to
		// reach the maximum/minimum scale (i64 range).
		let pow2 = base.powi(exponent);

		bd * pow2
	} else {
		// scale != 0, which means that integral_only is not set, so only base 10 and 16
		// are allowed.
		unreachable!();
	};
...
pub(crate) fn parse(
	...
	// Note: literals with ' and " prefixes are parsed earlier on in argument
	// parsing, before UTF-8 conversion.
	...
	// Parse an optional base prefix ("0b" / "0B" / "0" / "0x" / "0X"). "0" is octal
	// unless a fractional part is allowed in which case it is an insignificant
	// leading 0. A "0" prefix will not be consumed in case the parsable string
	// contains only "0": the leading extra "0" will have no influence on the
	// result.
	...
	// We only parse fractional and exponent part of the number in base 10/16
	...
	// If no digit has been parsed, check if this is a special value, or declare the
...
mod tests {
	...
	fn test_decimal_u64() {
		...
		assert!(matches!(u64::extended_parse(""), Err(ExtendedParserError::NotNumeric)));
		assert_eq!(
	...
	fn test_decimal_i64() {
		...
		assert_eq!(Ok(123), i64::extended_parse("+123"));
		assert_eq!(Ok(-123), i64::extended_parse("-123"));
		assert!(matches!(i64::extended_parse("--123"), Err(ExtendedParserError::NotNumeric)));
		...
		assert!(matches!(i64::extended_parse(""), Err(ExtendedParserError::NotNumeric)));
		assert!(matches!(i64::extended_parse("."), Err(ExtendedParserError::NotNumeric)));
	}
	...
	fn test_decimal_f64() {
		...
		assert_eq!(Ok(0.15), f64::extended_parse(".150000000000000000000000000231313"));
		assert_eq!(
		...
		assert_eq!(
			...
			Err(ExtendedParserError::PartialMatch(f64::NEG_INFINITY, "init".to_string()))
		);
		assert_eq!(
			f64::extended_parse("-infinity00"),
			Err(ExtendedParserError::PartialMatch(f64::NEG_INFINITY, "00".to_string()))
		);
		...
		assert!(matches!(f64::extended_parse("1.0e-9000"), Err(ExtendedParserError::Underflow(0.0))));
		assert!(matches!(
	...
	fn test_decimal_extended_big_decimal() {
		...
		// Careful, we usually cannot use From<f64> to get a precise ExtendedBigDecimal
		// as numbers like 123.15 cannot be exactly represented by a f64.
		assert_eq!(
			Ok(ExtendedBigDecimal::BigDecimal(BigDecimal::from_str("123").unwrap())),
			ExtendedBigDecimal::extended_parse("123")
		);
		assert_eq!(
			Ok(ExtendedBigDecimal::BigDecimal(BigDecimal::from_str("123.15").unwrap())),
			ExtendedBigDecimal::extended_parse("123.15")
		);
		assert_eq!(
			Ok(ExtendedBigDecimal::BigDecimal(BigDecimal::from_bigint(12315.into(), -98))),
			ExtendedBigDecimal::extended_parse("123.15e100")
		);
		assert_eq!(
			Ok(ExtendedBigDecimal::BigDecimal(BigDecimal::from_bigint(12315.into(), 102))),
		...
		assert!(matches!(ExtendedBigDecimal::extended_parse("nan"), Ok(ExtendedBigDecimal::Nan)));
		assert!(matches!(
			ExtendedBigDecimal::extended_parse("-NAN"),
			Ok(ExtendedBigDecimal::MinusNan)
		));
		assert_eq!(Ok(ExtendedBigDecimal::Infinity), ExtendedBigDecimal::extended_parse("InF"));
		assert_eq!(Ok(ExtendedBigDecimal::MinusInfinity), ExtendedBigDecimal::extended_parse("-iNf"));
		assert_eq!(Ok(ExtendedBigDecimal::zero()), ExtendedBigDecimal::extended_parse("0.0"));
		assert!(matches!(
		...
		assert!(matches!(
			...
			Err(ExtendedParserError::Overflow(ExtendedBigDecimal::MinusInfinity))
		));
		assert!(matches!(
		...
		/* Invalid numbers */
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse(""));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse("."));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse("e"));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse(".e"));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse("-e"));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse("+.e"));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse("e10"));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse("e-10"));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse("-e10"));
		assert_eq!(Err(ExtendedParserError::NotNumeric), ExtendedBigDecimal::extended_parse("+e10"));
	}
	...
	fn test_hexadecimal() {
		...
		// We cannot really check that 'e' is not a valid exponent indicator for hex
		// floats... but we can check that the number still gets parsed properly:
		// 0x0.8e5 is 0x8e5 / 16**3
		...
		assert_eq!(
			Ok(ExtendedBigDecimal::BigDecimal(BigDecimal::from_str("0.0625").unwrap())),
		...
		// Test very large exponents (they used to take forever as we kept all digits in
		// the past) Wolfram Alpha can get us (close to?) these values with a bit of
		// log trickery: 2**3000000000 = 10**log_10(2**3000000000) = 10**(3000000000 *
		// log_10(2)) TODO: We do lose a little bit of precision, and the last digits
		// are not be correct.
		assert_eq!(
            Ok(ExtendedBigDecimal::BigDecimal(
	...
	fn test_binary() {
		...
		assert_eq!(
			...
			Err(ExtendedParserError::PartialMatch(0f64, "b100.1".to_string()))
		);

		assert_eq!(
			ExtendedBigDecimal::extended_parse("0b100.1"),
			Err(ExtendedParserError::PartialMatch(ExtendedBigDecimal::zero(), "b100.1".to_string()))
		);

		assert_eq!(
			ExtendedBigDecimal::extended_parse("0b"),
			Err(ExtendedParserError::PartialMatch(ExtendedBigDecimal::zero(), "b".to_string()))
		);
		assert_eq!(
			ExtendedBigDecimal::extended_parse("0b."),
			Err(ExtendedParserError::PartialMatch(ExtendedBigDecimal::zero(), "b.".to_string()))
	...
	fn test_parsing_with_leading_whitespace() {
		...
		assert_eq!(Err(ExtendedParserError::NotNumeric), i64::extended_parse("\u{2029}-0x9"));

		// Ensure that whitespace after the number has "started" is not allowed
		assert_eq!(Err(ExtendedParserError::NotNumeric), i64::extended_parse("- 0x9"));
	}
}
