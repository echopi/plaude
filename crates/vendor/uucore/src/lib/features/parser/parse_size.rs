...
impl<'parser> Parser<'parser> {
	...
	/// Change b_byte_count of the parser - to treat "b" as a "byte count"
	/// instead of "block"
	...
	/// and GNU test suite checks this behavior for some utils (`split` for
	/// example)
	...
	/// Same as `parse_u64_max()`, except for u128, i.e. returns `u128::MAX` on
	/// overflow
...
#[deprecated = "Please use parse_size_u64(size: &str) -> Result<u64, ParseSizeError> OR \
                parse_size_u128(size: &str) -> Result<u128, ParseSizeError> instead."]
...
impl ParseSizeError {
	...
	fn size_too_big(s: &str) -> Self {
		...
		// has to be handled in the respective uutils because strings differ, e.g.:
		//
		// head:     invalid number of bytes: '1Y': Value too large for defined data
		// type tail:     invalid number of bytes: '1Y': Value too large for defined
		// data type split:    invalid number of bytes: '1Y': Value too large for
		// defined data type truncate:          Invalid number: '1Y': Value too large
		// for defined data type stdbuf:               invalid mode '1Y': Value too
		// large for defined data type sort:     -S argument '1Y' too large
		// du:       -B argument '1Y' too large
		// od:       -N argument '1Y' too large
		// etc.
		//
		// stderr on macos (brew - GNU coreutils 8.32) also differs for the same
		// version, e.g.: ghead:   invalid number of bytes: '1Y': Value too large to
		// be stored in data type gtail:   invalid number of bytes: '1Y': Value too
		// large to be stored in data type
		Self::SizeTooBig(format!("{}: Value too large for defined data type", s.quote()))
	}
}
...
mod tests {
	...
	fn overflow_to_max_u128() {
		assert_eq!(Ok(12_379_400_392_853_802_748_991_242_240), parse_size_u128_max("10R"));
		assert_eq!(Ok(12_676_506_002_282_294_014_967_032_053_760), parse_size_u128_max("10Q"));
	...
	fn x64() {
		...
		assert_eq!(Ok(1_237_940_039_285_380_274_899_124_224), parse_size_u128("1R"));
		assert_eq!(Ok(1_267_650_600_228_229_401_496_703_205_376), parse_size_u128("1Q"));
		...
		assert_eq!(Ok(2_000_000_000_000_000_000_000_000), parse_size_u128("2YB"));
		assert_eq!(Ok(2_000_000_000_000_000_000_000_000_000), parse_size_u128("2RB"));
		assert_eq!(Ok(2_000_000_000_000_000_000_000_000_000_000), parse_size_u128("2QB"));
	}
	...
	fn parse_size_options() {
		...
		assert_eq!(Ok(1024 * 1024), parser.parse("1M"));
		assert_eq!(Ok(1024 * 1024 * 1024), parser.parse("1G"));
		assert_eq!(Ok(1_237_940_039_285_380_274_899_124_224), parser.parse_u128("1R"));
		assert_eq!(Ok(1_267_650_600_228_229_401_496_703_205_376), parser.parse_u128("1Q"));
