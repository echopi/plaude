...
use std::{borrow::Cow, ffi::OsString, fs::Metadata, time::SystemTime};
...
impl MountInfo {
	...
	fn new(file_name: &str, raw: &[&[u8]]) -> Option<Self> {
		...
		Some(Self { dev_id, dev_name, fs_type, mount_root, mount_dir, mount_option, remote, dummy })
	}
...
pub fn read_fs_list() -> UResult<Vec<MountInfo>> {
	...
	#[cfg(windows)]
	{
		...
		// As recommended in the MS documentation, retrieve the first volume before the
		// others
		let find_handle =
...
impl FsUsage {
	...
	pub fn new(path: &Path) -> UResult<Self> {
		...
		if 0 == success {
			let msg = format!("GetVolumePathNamesForVolumeNameW failed: {}", IOError::last_os_error());
			return Err(USimpleError::new(EXIT_ERR, msg));
		}
...
impl FsMeta for StatFs {
	...
	fn fs_type(&self) -> i64 {
		...
		#[cfg(any(target_env = "musl", all(target_os = "android", target_pointer_width = "64"),))]
...
mod tests {
	...
	fn test_mountinfo_dir_non_unicode() {
		...
		assert_eq!(info.mount_dir, crate::os_str_from_bytes(b"/mnt/some-\xc0-dir-\xf3").unwrap());
		...
		assert_eq!(info.mount_dir, crate::os_str_from_bytes(b"/mnt/some- -dir-\xf3").unwrap());
	}
