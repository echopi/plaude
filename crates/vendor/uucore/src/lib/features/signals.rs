...
// spell-checker:ignore (vars/api) fcntl setrlimit setitimer rubout pollable
// sysconf pgrp GETFD pfds revents POLLRDBAND POLLERR spell-checker:ignore
// (vars/signals) ABRT ALRM CHLD SEGV SIGABRT SIGALRM SIGBUS SIGCHLD SIGCONT
// SIGDANGER SIGEMT SIGFPE SIGHUP SIGILL SIGINFO SIGINT SIGIO SIGIOT SIGKILL
// SIGMIGRATE SIGMSG SIGPIPE SIGPRE SIGPROF SIGPWR SIGQUIT SIGSEGV SIGSTOP
// SIGSYS SIGTALRM SIGTERM SIGTRAP SIGTSTP SIGTHR SIGTTIN SIGTTOU SIGURG SIGUSR
// SIGVIRT SIGVTALRM SIGWINCH SIGXCPU SIGXFSZ STKFLT PWR THR TSTP TTIN TTOU VIRT
// VTALRM XCPU XFSZ SIGCLD SIGPOLL SIGWAITING SIGAIOCANCEL SIGLWP SIGFREEZE
// SIGTHAW SIGCANCEL SIGLOST SIGXRES SIGJVM SIGRTMIN SIGRT SIGRTMAX TALRM
// AIOCANCEL XRES RTMIN RTMAX LTOSTOP

//! This module provides a way to handle signals in a platform-independent way.
//! It provides a way to convert signal names to their corresponding values and
//! vice versa. It also provides a way to ignore the SIGINT signal and enable
//! pipe errors.
...
pub static ALL_SIGNALS: [&str; 32] = [
	...
	"USR2", "PIPE", "ALRM", "TERM", "STKFLT", "CHLD", "CONT", "STOP", "TSTP", "TTIN", "TTOU", "URG",
...
pub static ALL_SIGNALS: [&str; 33] = [
	"EXIT", "HUP", "INT", "QUIT", "ILL", "TRAP", "ABRT", "EMT", "FPE", "KILL", "BUS", "SEGV", "SYS",
	"PIPE", "ALRM", "TERM", "URG", "STOP", "TSTP", "CONT", "CHLD", "TTIN", "TTOU", "IO", "XCPU",
	"XFSZ", "VTALRM", "PROF", "WINCH", "INFO", "USR1", "USR2", "THR",
];
...
pub static ALL_SIGNALS: [&str; 37] = [
	...
	"MSG", "WINCH", "PWR", "USR1", "USR2", "PROF", "DANGER", "VTALRM", "MIGRATE", "PRE", "VIRT",
	"TALRM",
];
...
pub static ALL_SIGNALS: [&str; 32] = [
	"EXIT", "HUP", "INT", "QUIT", "ILL", "TRAP", "ABRT", "EMT", "FPE", "KILL", "BUS", "SEGV", "SYS",
	"PIPE", "ALRM", "TERM", "URG", "STOP", "TSTP", "CONT", "CHLD", "TTIN", "TTOU", "IO", "XCPU",
	"XFSZ", "VTALRM", "PROF", "WINCH", "PWR", "USR1", "USR2",
];
...
pub fn enable_pipe_errors() -> Result<(), Errno> {
	// We pass the error as is, the return value would just be Ok(SigDfl), so we can
	// safely ignore it. SAFETY: this function is safe as long as we do not use a
	// custom SigHandler -- we use the default one.
	unsafe { signal(SIGPIPE, SigDfl) }.map(|_| ())
}

/// Ignores SIGPIPE signal (broken pipe errors are returned instead of
/// terminating). Use this to override the default SIGPIPE handling when you
/// need to handle broken pipe errors gracefully (e.g., tee with
/// --output-error).
#[cfg(unix)]
pub fn disable_pipe_errors() -> Result<(), Errno> {
	// SAFETY: this function is safe as long as we do not use a custom SigHandler --
...
pub fn ignore_interrupts() -> Result<(), Errno> {
	// We pass the error as is, the return value would just be Ok(SigIgn), so we can
	// safely ignore it. SAFETY: this function is safe as long as we do not use a
	// custom SigHandler -- we use the default one.
...
// Detect closed stdin/stdout before Rust reopens them as /dev/null (see issue
// #2873)
#[cfg(unix)]
...
// SIGPIPE state capture - captures whether SIGPIPE was ignored at process
// startup
#[cfg(unix)]
...
/// Captures stdio and SIGPIPE state at process initialization, before main()
/// runs.
///
/// # Safety
...
pub unsafe extern "C" fn capture_startup_state() {
	...
	unsafe {
		STDIN_WAS_CLOSED
			.store(libc::fcntl(libc::STDIN_FILENO, libc::F_GETFD) == -1, Ordering::Relaxed);
		STDOUT_WAS_CLOSED
			.store(libc::fcntl(libc::STDOUT_FILENO, libc::F_GETFD) == -1, Ordering::Relaxed);
		STDERR_WAS_CLOSED
			.store(libc::fcntl(libc::STDERR_FILENO, libc::F_GETFD) == -1, Ordering::Relaxed);
	}
...
macro_rules! init_startup_state_capture {
	() => {
		...
		static CAPTURE_STARTUP_STATE: unsafe extern "C" fn() = $crate::signals::capture_startup_state;
...
pub fn ensure_stdout_not_broken() -> std::io::Result<bool> {
	...
	// Use ZERO timeout to return immediately - we just want to check the current
	// state.
	...
	// res == 0 means no events ready (timeout reached immediately with ZERO
	// timeout). This means the pipe is healthy (not broken).
...
fn signal_by_long_name() {
	for (value, signal) in ALL_SIGNALS.iter().enumerate() {
		assert_eq!(signal_by_name_or_value(&format!("SIG{signal}")), Some(value));
	}
}
