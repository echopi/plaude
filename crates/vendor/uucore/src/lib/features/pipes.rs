...
/// from the first.
/// used for resolving the limitation for splice: one of a input or output
/// should be pipe
#[inline]
...
pub fn splice(source: &impl AsFd, target: &impl AsFd, len: usize) -> std::io::Result<usize> {
	Ok(rustix::pipe::splice(source, None, target, None, len, SpliceFlags::empty())?)
}
...
///
/// `splice` to /dev/null is faster than `read` when we skip or count the input
/// which is not able to seek
#[inline]
#[cfg(any(target_os = "linux", target_os = "android"))]
