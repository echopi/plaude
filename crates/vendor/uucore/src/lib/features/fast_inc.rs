...
///
/// Add inc to the string val[start..end]. This operates on ASCII digits,
/// assuming val and inc are well formed.
///
...
pub fn fast_inc(val: &mut [u8], start: &mut usize, end: usize, inc: &[u8]) {
	...
	for inc_pos in (0..inc.len()).rev() {
		// The decrement operation would also panic in debug mode, print a message for
		// developer convenience.
		debug_assert!(pos > 0, "Buffer overflowed, make sure you allocate val with enough headroom.");
		pos -= 1;
...
pub fn fast_inc_one(val: &mut [u8], start: &mut usize, end: usize) {
	...
	// The following decrement operation would also panic in debug mode, print a
	// message for developer convenience.
	debug_assert!(*start > 0, "Buffer overflowed, make sure you allocate val with enough headroom.");
...
#[cfg(test)]
mod tests {
	use crate::fast_inc::{fast_inc, fast_inc_one};
