...
/// * `xattrs` - A HashMap containing attribute names and their corresponding
///   values.
///
/// # Returns
///
...
/// Checks if a file has an Access Control List (ACL) based on its extended
/// attributes.
///
/// # Arguments
///
...
/// # Returns
///
/// `true` if the file has extended attributes (indicating an ACL), `false`
/// otherwise.
...
/// Checks if a file has an Access Control List (ACL) named
/// "security.capability" based on its extended attributes.
///
/// # Arguments
///
/// * `file` - A reference to the path of the file.
...
/// # Returns
///
/// `true` if the file has an extended attribute named "security.capability",
/// `false` otherwise.
...
/// Returns the permissions bits of a file or directory which has Access Control
/// List (ACL) entries based on its extended attributes (Only works for linux)
///
/// # Arguments
///
...
/// # Returns
///
/// `u32`  the perm bits of a file having extended attributes of type
/// 'system.posix_acl_default' with permissions otherwise returns a 0 if perm
/// bits are 0 or the file has no extended attributes
pub fn get_acl_perm_bits_from_xattr<P: AsRef<Path>>(source: P) -> u32 {
	// TODO: Modify this to work on non linux unix systems.

	// Only default acl entries get inherited by objects under the path i.e. if
	// child directories will have their permissions modified.
	if let Ok(entries) = retrieve_xattrs(source) {
		...
		if let Some(value) = entries.get(&OsString::from("system.posix_acl_default")) {
			...
			// value follows a starts with a 4 byte header, and then has posix_acl_entries,
			// each posix_acl_entry is separated by a u32 sequence i.e. 0xFFFFFFFF
			//
			...
			//
			// The value of the header is 0x0002, so we skip the first four bytes of the
			// value and process the rest
...
mod tests {
	...
	fn test_get_perm_bits_from_xattrs() {
		...
		// Reference: `<https://github.com/torvalds/linux/blob/master/include/uapi/linux/posix_acl_xattr.h>`
		// The id is undefined i.e. -1 which in u32 is 0xFFFFFFFF and tag and perm bits
		// as given in the header file.
		// Reference: `<https://github.com/torvalds/linux/blob/master/include/uapi/linux/posix_acl.h>`
		//
		//
		// There is a bindgen bug which generates the ACL_OTHER constant whose value is
		// 0x20 into 32. which when the bug is fixed will need to be changed back to
		// 20 from 32 in the vec 'test_value'.
		//
		// Reference `<https://github.com/rust-lang/rust-bindgen/issues/2926>`
		//
		// The test_value vector is the header 0x0002 followed by tag and permissions
		// for user_obj , tag and permissions and for group_obj and finally the tag
		// and permissions for ACL_OTHER. Each entry has undefined id as mentioned
		// above.
		//
		//

		let test_value = vec![
			2, 0, 0, 0, 1, 0, 7, 0, 255, 255, 255, 255, 4, 0, 0, 0, 255, 255, 255, 255, 32, 0, 0, 0,
			255, 255, 255, 255,
		];
	...
	fn test_file_has_acl() {
		...
		// However, individual file systems might still implement additional namespaces
		// according to https://man.freebsd.org/cgi/man.cgi?query=extattr&sektion=9&manpath=FreeBSD+14.3-RELEASE+and+Ports
