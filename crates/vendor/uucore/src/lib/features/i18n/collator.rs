...
/// Compute the ICU collation sort key for the given input bytes and append it
/// to `buf`. This allows pre-computing sort keys once per line, then comparing
/// them with simple byte comparison during sorting (much faster than calling
/// `compare_utf8` per comparison).
