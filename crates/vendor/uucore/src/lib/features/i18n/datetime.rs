...
use icu_calendar::{
	Date,
	cal::{Buddhist, Ethiopian, Iso, Persian},
};
...
pub enum CalendarType {
	...
	Buddhist,
	/// Persian Solar Hijri calendar (Persian/Farsi locales) - subtracts 621/622
	/// years
	Persian,
...
pub fn localize_format_string(format: &str, date: JiffDate) -> String {
	...
	if fmt.contains("%b") || fmt.contains("%h") {
		if let Ok(f) = DateTimeFormatter::try_new(locale_prefs, fieldsets::M::medium()) {
			// ICU's medium format may include trailing periods (e.g., "febr." for
			// Hungarian), which when combined with locale format strings that also add
			// periods after %b (e.g., "%Y. %b. %d") results in double periods
			// ("febr.."). The standard C/POSIX locale via nl_langinfo returns
			// abbreviations WITHOUT trailing periods, so we strip them here for
			// consistency.
...
///
/// Each entry corresponds to months January (index 0) through December (index
/// 11). Returns `None` for C/POSIX locale (caller should use English defaults).
/// This matches the GNU coreutils approach of storing uppercased,
/// blank-stripped names.
...
/// Unix implementation using nl_langinfo for exact match with `locale abmon`
/// output.
#[cfg(any(
	target_os = "linux",
	target_vendor = "apple",
...
fn get_locale_months_inner() -> Option<[Vec<u8>; 12]> {
	...
	for (i, &item) in abmon_items.iter().enumerate() {
		// SAFETY: nl_langinfo returns a valid C string pointer for valid nl_item
		// values.
...
mod tests {
	...
	/// Confirm that M::short() gives numeric months and M::long() gives full
	/// names, so M::medium() is the only correct choice for abbreviated month
	/// names.
	#[test]
	...
	fn test_calendar_type_detection() {
		use icu_locale::locale;
		assert_eq!(get_locale_calendar_type(&locale!("th-TH")), CalendarType::Buddhist);
		assert_eq!(get_locale_calendar_type(&locale!("fa-IR")), CalendarType::Persian);
		assert_eq!(get_locale_calendar_type(&locale!("am-ET")), CalendarType::Ethiopian);
		assert_eq!(get_locale_calendar_type(&locale!("en-US")), CalendarType::Gregorian);
	}
}
