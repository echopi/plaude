...
impl FromStr for Range {
	...
	fn from_str(s: &str) -> Result<Self, &'static str> {
		...
		Ok(match s.split_once('-') {
			...
			Some((low, "")) => Self { low: parse(low)?, high: usize::MAX - 1 },
			Some(("", high)) => Self { low: 1, high: parse(high)? },
			Some((low, high)) => {
...
pub fn complement(ranges: &[Range]) -> Vec<Range> {
	...
	for range in ranges {
		if range.low > prev_high + 1 {
			complements.push(Range { low: prev_high + 1, high: range.low - 1 });
		}
		prev_high = range.high;
	}

	if prev_high < usize::MAX - 1 {
		complements.push(Range { low: prev_high + 1, high: usize::MAX - 1 });
	}
...
mod test {
	...
	fn merging() {
		...
		m(vec![r(1, 3), r(6, 7), r(2, 4), r(6, 7)], &[r(1, 4), r(6, 7)]);

		// One giant
		m(vec![r(110, 120), r(10, 20), r(100, 200), r(130, 140), r(150, 160)], &[
			r(10, 20),
			r(100, 200),
		]);
		...
		m(vec![r(10, 20), r(30, 40), r(50, 60), r(20, 30)], &[r(10, 40), r(50, 60)]);
	...
	fn complementing() {
		...
		// With start
		assert_eq!(complement(&[r(1, 3), r(6, 10)]), vec![r(4, 5), r(11, usize::MAX - 1)]);

		// With end
		assert_eq!(complement(&[r(2, 4), r(6, usize::MAX - 1)]), vec![r(1, 1), r(5, 5)]);
	...
	fn test_from_str() {
		...
		assert_eq!(Range::from_str("5-3"), Err("high end of range less than low end"));
		assert_eq!(Range::from_str("-"), Err("invalid range with no endpoint"));
		assert_eq!(Range::from_str("3-"), Ok(Range { low: 3, high: usize::MAX - 1 }));
		assert_eq!(Range::from_str("-5"), Ok(Range { low: 1, high: 5 }));
		assert_eq!(Range::from_str("0"), Err("fields and positions are numbered from 1"));

		let max_value = format!("{}", usize::MAX);
		assert_eq!(Range::from_str(&max_value), Err("byte/character offset is too large"));
	}
}
