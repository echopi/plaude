...
use crate::{
	...
	quoting_style::{
		c_quoter::CQuoter,
		literal_quoter::LiteralQuoter,
		shell_quoter::{EscapedShellQuoter, NonEscapedShellQuoter},
	},
};
...
impl QuotingStyle {
	pub const C_DOUBLE: Self = Self::C { quotes: Quotes::Double };
	pub const C_NO_QUOTES: Self = Self::C { quotes: Quotes::None };
	...
	pub const SHELL_ESCAPE_QUOTE: Self =
		Self::Shell { escape: true, always_quote: true, show_control: false };
	pub const SHELL_QUOTE: Self =
	...
	pub fn show_control(self, show_control: bool) -> Self {
		use QuotingStyle::*;
		match self {
			Shell { escape, always_quote, .. } => Shell { escape, always_quote, show_control },
...
fn escape_name_inner(
	...
	let mut quoter: Box<dyn Quoter> = match style {
		...
		QuotingStyle::Shell { escape: true, always_quote, .. } => {
			Box::new(EscapedShellQuoter::new(name, always_quote, dirname, name.len()))
		},
		QuotingStyle::Shell { escape: false, always_quote, show_control } => {
			Box::new(NonEscapedShellQuoter::new(name, show_control, always_quote, dirname, name.len()))
		},
	};
...
mod tests {
	...
	fn get_style(s: &str) -> QuotingStyle {
		match s {
			"literal" => QuotingStyle::Literal { show_control: false },
	...
	fn check_names_encoding(encoding: UEncoding, name: &str, map: &[(&str, &str)]) {
		assert_eq!(
			...
			check_names_inner(encoding, name.as_bytes(), map)
				.iter()
				.map(|bytes| std::str::from_utf8(bytes).expect("valid str goes in, valid str comes out"))
				.collect::<Vec<&str>>()
		);
	}
	...
	fn test_quotes() {
		...
		check_names_both("one'two\"three", &[
	...
	fn test_control_chars() {
		...
		// The first 16 ASCII control characters. NUL is also included, even though it
		// is of no importance for file names.
		check_names_both("\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F", &[
			("????????????????", "literal"),
			("\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F", "literal-show"),
			("\\000\\001\\002\\003\\004\\005\\006\\a\\b\\t\\n\\v\\f\\r\\016\\017", "escape"),
			("\"\\000\\001\\002\\003\\004\\005\\006\\a\\b\\t\\n\\v\\f\\r\\016\\017\"", "c"),
			("'????????????????'", "shell"),
			("'\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F'", "shell-show"),
			("'????????????????'", "shell-always"),
			(
		...
		check_names_both("\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C\x1D\x1E\x1F", &[
			("????????????????", "literal"),
			("\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C\x1D\x1E\x1F", "literal-show"),
			(
			...
			("????????????????", "shell"),
			("\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C\x1D\x1E\x1F", "shell-show"),
			("'????????????????'", "shell-always"),
			(
		...
		check_names_encoding(UEncoding::Utf8, test_str, &[
			("????????????????", "literal"),
			("????????????????", "shell"),
			("'????????????????'", "shell-always"),
		]);
		check_names_encoding(UEncoding::Ascii, test_str, &[
			("????????????????????????????????", "literal"),
			("????????????????????????????????", "shell"),
			("'????????????????????????????????'", "shell-always"),
		]);
	...
	fn test_non_unicode_bytes() {
		...
		check_names_raw_both(&[continuation], &[
			(b"?", "literal"),
			(b"\xA7", "literal-show"),
			(b"\\247", "escape"),
			(b"\"\\247\"", "c"),
		...
		check_names_raw_both(input, &[
			(b"\xC2\xA7", "literal-show"),
			(b"\xC2\xA7", "shell-show"),
			(b"'\xC2\xA7'", "shell-always-show"),
		]);
		// Different expected output for UTF-8 and ASCII in these cases.
		check_names_encoding_raw(UEncoding::Utf8, input, &[
			(b"\xC2\xA7", "literal"),
			(b"\xC2\xA7", "escape"),
			(b"\"\xC2\xA7\"", "c"),
			(b"\xC2\xA7", "shell"),
		...
		check_names_encoding_raw(UEncoding::Ascii, input, &[
			(b"??", "literal"),
			(b"\\302\\247", "escape"),
			(b"\"\\302\\247\"", "c"),
			(b"??", "shell"),
		...
		check_names_raw_both(&[continuation, ascii], &[
			(b"?_", "literal"),
			(b"\xA7_", "literal-show"),
			(b"\\247_", "escape"),
			(b"\"\\247_\"", "c"),
		...
		check_names_raw_both(&[ascii, continuation], &[
			(b"_?", "literal"),
		...
		check_names_raw_both(&[ascii, continuation, ascii], &[
			(b"_?_", "literal"),
		...
		check_names_raw_both(&[continuation, ascii, continuation], &[
			(b"?_?", "literal"),
		...
		check_names_raw_both(
			...
			&[
				(b"_?_??_???_????_", "literal"),
				(b"_\xC0_\xA7\xA7_\xA7\xA7\xA7_\xA7\xA7\xA7\xA7_", "literal-show"),
				(b"_\\300_\\247\\247_\\247\\247\\247_\\247\\247\\247\\247_", "escape"),
				(b"\"_\\300_\\247\\247_\\247\\247\\247_\\247\\247\\247\\247_\"", "c"),
				(b"_?_??_???_????_", "shell"),
				(b"_\xC0_\xA7\xA7_\xA7\xA7\xA7_\xA7\xA7\xA7\xA7_", "shell-show"),
				(b"'_?_??_???_????_'", "shell-always"),
				(b"'_\xC0_\xA7\xA7_\xA7\xA7\xA7_\xA7\xA7\xA7\xA7_'", "shell-always-show"),
				(
		...
		check_names_raw_both(&[first2byte, ascii], &[
			(b"?_", "literal"),
			(b"\xC2_", "literal-show"),
			(b"\\302_", "escape"),
			(b"\"\\302_\"", "c"),
		...
		check_names_raw_both(&[first3byte, continuation, ascii], &[
			(b"??_", "literal"),
		...
		check_names_raw_both(&[first4byte, continuation, continuation, ascii], &[
			(b"???_", "literal"),
	...
	fn test_tilde_and_hash() {
		...
		check_names_both("some~name", &[("some~name", "shell"), ("some~name", "shell-escape")]);
		check_names_both("name~", &[("name~", "shell"), ("name~", "shell-escape")]);
	...
	fn test_quoting_style_display() {
		...
		let style = QuotingStyle::Literal { show_control: false };
