...
// This implementation is heavily inspired by the std::char::EscapeDefault
// implementation in the Rust standard library. This custom implementation is
// needed because the characters \a, \b, \e, \f & \v are not recognized by Rust.
...
impl EscapeOctal {
	fn from_char(c: char) -> Self {
		...
		Self { c: buf, idx: 2, state: EscapeOctalState::FirstBackslash }
	}

	fn from_byte(b: u8) -> Self {
		Self { c: [0, b], idx: 2, state: EscapeOctalState::LastBackslash }
	}
}
...
impl EscapedChar {
	...
	pub fn new_octal(b: u8) -> Self {
		Self { state: EscapeState::Octal(EscapeOctal::from_byte(b)) }
	}
	...
	pub fn hide_control(self) -> Self {
		match self.state {
			EscapeState::Char(c) if c.is_control() => Self { state: EscapeState::Char('?') },
			_ => self,
		}
	}
}
