...
impl<'a> NonEscapedShellQuoter<'a> {
	pub fn new(
		...
		Self { reference, quotes, show_control, must_quote, buffer: Vec::with_capacity(size_hint) }
	}
}
...
mod tests {
	...
	fn test_initial_quoting() {
		...
		assert_eq!(initial_quoting(b"\x01", false, false, true), (Quotes::Single, true));

		// Control + quote uses single quotes (segmented) in escape mode
		assert_eq!(initial_quoting(b"\x01'\x01", false, false, true), (Quotes::Single, true));

		// Simple quote uses double quotes in escape mode
		assert_eq!(initial_quoting(b"a'b", false, false, true), (Quotes::Double, true));

		// Shell special chars force single quotes in escape mode
		assert_eq!(initial_quoting(b"test$var", false, false, true), (Quotes::Single, true));
		assert_eq!(initial_quoting(b"test\nline", false, false, true), (Quotes::Single, true));

		// Empty string forces quotes in escape mode
		assert_eq!(initial_quoting(b"", false, false, true), (Quotes::Single, true));

		// Always quote flag works in escape mode
		assert_eq!(initial_quoting(b"normal", false, true, true), (Quotes::Single, true));

		// Normal text doesn't need quoting in escape mode
		assert_eq!(initial_quoting(b"hello", false, false, true), (Quotes::Single, false));

		// Dirname affects colon handling in escape mode
		assert_eq!(initial_quoting(b"dir:name", true, false, true), (Quotes::Single, true));
		assert_eq!(initial_quoting(b"file:name", false, false, true), (Quotes::Single, false));

		// Control chars ignored in non-escape mode
		assert_eq!(initial_quoting(b"\x01", false, false, false), (Quotes::Single, false));
	}
}
