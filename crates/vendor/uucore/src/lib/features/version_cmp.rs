...
mod tests {
	...
	fn test_version_cmp() {
		...
		assert_eq!(version_cmp(b"file12-suffix", b"file12-suffix"), Ordering::Equal);

		assert_eq!(version_cmp(b"file12-suffix24", b"file12-suffix24"), Ordering::Equal);
		...
		assert_eq!(version_cmp(b"20", b"20"), Ordering::Equal, "Equal numbers are equal");

		assert_eq!(version_cmp(b"15", b"200"), Ordering::Less, "Small numbers are smaller");
		...
		assert_eq!(
			version_cmp(b"aa2000000000000000000000bb", b"aa002000000000000000000001bb"),
			Ordering::Less,
			"Numbers larger than u64::MAX are handled correctly without crashing"
		);

		assert_eq!(
			version_cmp(b"aa2000000000000000000000bb", b"aa002000000000000000000000bb"),
			Ordering::Equal,
		...
		assert_eq!(version_cmp(b"a~", b"ab"), Ordering::Less, "A tilde is before other letters");

		assert_eq!(version_cmp(b"a~", b"a"), Ordering::Less, "A tilde is before the line end");
		assert_eq!(version_cmp(b"~", b""), Ordering::Greater, "A tilde is after the empty string");
		assert_eq!(
			version_cmp(b".f", b".1"),
			Ordering::Greater,
