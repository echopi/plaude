...
//!
//! ```no_run
//! use uucore::hardware::{CpuFeatures, HasHardwareFeatures as _, SimdPolicy};
//!
...
///
/// Respects GLIBC_TUNABLES environment variable to disable specific CPU
/// features. This is used by GNU utilities to allow users to disable hardware
/// acceleration.
#[derive(Debug, Clone)]
...
impl SimdPolicy {
	...
	fn detect_impl() -> Self {
		...
		Self { disabled_by_env, hardware_features }
	}

	/// Returns true if any SIMD feature remains enabled after applying
	/// GLIBC_TUNABLES.
	...
	for entry in tunables.split(':') {
		...
		for token in raw_value.split(',') {
			...
			if let Some(feature) = token.strip_prefix('-') {
				let feature = HardwareFeature::try_from(feature.trim().to_ascii_uppercase().as_str());
