...
//! 		.after_help(format!("{long_usage}\n{}", backup_control::BACKUP_CONTROL_LONG_HELP))
//! 		.get_matches_from(vec!["command", "--backup=t", "--suffix=bak~"]);
//!
...
//!
//! 	let backup_path = backup_control::get_backup_path(backup_mode, target_path, &backup_suffix);
//!
...
pub const BACKUP_CONTROL_LONG_HELP: &str = "The backup suffix is '~', unless set with --suffix or \
...
impl UError for BackupError {
	...
	fn usage(&self) -> bool {
		// Suggested by clippy.
		matches!(self, Self::InvalidArgument(_, _) | Self::AmbiguousArgument(_, _))
	}
}
...
impl Display for BackupError {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			Self::InvalidArgument(arg, origin) => {
				write!(f, "invalid argument {} for '{origin}'\n{VALID_ARGS_HELP}", arg.quote(),)
			},
			Self::AmbiguousArgument(arg, origin) => {
				write!(f, "ambiguous argument {} for '{origin}'\n{VALID_ARGS_HELP}", arg.quote(),)
			},
...
/// 3. By using the default '~' if none of the others apply, or if they
///    contained slashes
///
...
/// 		.arg(backup_control::arguments::backup())
/// 		.arg(backup_control::arguments::backup_no_args())
/// 		.get_matches_from(vec!["command", "-b", "--backup=t"]);
///
...
/// extern crate uucore;
/// use clap::{Arg, ArgMatches, Command};
/// use uucore::backup_control::{self, BackupError, BackupMode};
///
/// fn main() {
/// 	let matches = Command::new("command")
/// 		.arg(backup_control::arguments::backup())
/// 		.arg(backup_control::arguments::backup_no_args())
/// 		.get_matches_from(vec!["command", "-b", "--backup=n"]);
///
...
/// Returns true if the source file is likely to be the simple backup file for
/// the target file.
///
/// # Arguments
///
...
mod tests {
	...
	fn test_suffix_rejects_path_traversal() {
		let _dummy = TEST_MUTEX.lock().unwrap();
		let matches = make_app().get_matches_from(vec!["command", "-b", "--suffix", "_/../../dest"]);
	...
	fn test_simple_backup_path() {
		assert_eq!(simple_backup_path(Path::new(""), ".bak"), PathBuf::from(".bak"));
		assert_eq!(simple_backup_path(Path::new("/"), ".bak"), PathBuf::from("/.bak"));
		assert_eq!(
