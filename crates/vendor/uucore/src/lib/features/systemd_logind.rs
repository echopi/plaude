...
//
// spell-checker:ignore logind libsystemd btime unref RAII testuser GETPW
// sysconf
...
mod ffi {
	...
	unsafe extern "C" {
		...
		pub fn sd_session_get_type(session: *const c_char, session_type: *mut *mut c_char) -> c_int;
...
mod login {
	...
	pub fn get_session_start_time(session_id: &str) -> Result<u64, Box<dyn std::error::Error>> {
		...
		if result < 0 {
			return Err(
				format!("sd_session_get_start_time failed for session '{session_id}': {result}",)
					.into(),
			);
		}
	...
	pub fn get_session_remote_host(
		...
		if result < 0 {
			return Err(
				format!("sd_session_get_remote_host failed for session '{session_id}': {result}",)
					.into(),
			);
		}
	...
	pub fn get_session_display(
		...
		if result < 0 {
			return Err(
				format!("sd_session_get_display failed for session '{session_id}': {result}",).into(),
			);
		}
	...
	pub fn get_session_type(session_id: &str) -> Result<Option<String>, Box<dyn std::error::Error>> {
		let session_cstring = CString::new(session_id)?;
		let mut type_ptr: *mut libc::c_char = ptr::null_mut();

		let result = unsafe { ffi::sd_session_get_type(session_cstring.as_ptr(), &raw mut type_ptr) };
	...
	pub fn get_session_seat(session_id: &str) -> Result<Option<String>, Box<dyn std::error::Error>> {
		let session_cstring = CString::new(session_id)?;
		let mut seat_ptr: *mut libc::c_char = ptr::null_mut();

		let result = unsafe { ffi::sd_session_get_seat(session_cstring.as_ptr(), &raw mut seat_ptr) };
	...
	/// GNU coreutils uses the mtime of /var/lib/systemd/random-seed as a
	/// heuristic for boot time when utmp is unavailable, rather than querying
	/// systemd's authoritative KernelTimestamp. This creates inconsistency:
	/// `uptime -s` shows the actual kernel boot time while `who -b` shows ~1
	/// minute later when systemd services start.
	///
	/// Ideally, both should use the same source (KernelTimestamp) for semantic
	/// consistency. Consider proposing to GNU coreutils to use systemd's
	/// KernelTimestamp property instead.
...
pub fn read_login_records() -> UResult<Vec<SystemdLoginRecord>> {
	...
	// Sort sessions consistently for reproducible output (reverse for TTY sessions
	// first)
	sessions.sort();
	sessions.reverse();
	...
	for session_id in sessions {
		...
		let start_time = login::get_session_start_time(&session_id)
			.map_or(UNIX_EPOCH, |usec| UNIX_EPOCH + std::time::Duration::from_micros(usec));
		...
		if !seat.is_empty() && !tty.is_empty() {
			...
			records.push(create_record(display, String::new(), user, session_id, host));
		}
	}
...
impl SystemdUtmpxCompat {
	...
	pub fn tty_device(&self) -> String {
		// Return raw device name for device access if available, otherwise formatted
		// seat_or_tty
...
impl SystemdUtmpxIter {
	...
	pub fn new() -> UResult<Self> {
		...
		Ok(Self { records, current_index: 0 })
	}

	/// Create empty iterator (for when systemd initialization fails)
	pub fn empty() -> Self {
		Self { records: Vec::new(), current_index: 0 }
	}
...
mod tests {
	...
	fn test_iterator_with_mock_data() {
		...
		let mut iter = SystemdUtmpxIter { records: mock_records, current_index: 0 };
	...
	fn test_get_all_records() {
		...
		let iter = SystemdUtmpxIter { records: mock_records, current_index: 0 };
