...
/// Helper function to set up a temporary test file and leak the temporary
/// directory so it persists for the duration of the benchmark
...
pub mod text_data {
	...
	/// Helper function to generate test data from a list of words without number
	/// suffix
	...
	/// Generate test data with accented characters that require locale-aware
	/// sorting
...
pub mod binary_data {
	...
	///
	/// Creates a file of the specified size (in MB) filled with the given byte
	/// pattern. This is useful for benchmarking utilities that work with large
	/// binary files like dd, cp, etc.
...
pub mod fs_utils {
	...
	///
	/// This is a convenience function for cleaning up between benchmark
	/// iterations. It handles both files and directories, and is a no-op if the
	/// path doesn't exist.
...
pub mod fs_tree {
	...
	///
	/// Creates a tree with specified depth, number of directories per level, and
	/// files per directory. This creates a realistic filesystem structure for
	...
	///
	/// This creates a flat structure with many files and directories at a
	/// shallow depth, useful for benchmarking operations that need to traverse
	/// many entries quickly.
	...
	///
	/// This creates a linear chain of deeply nested directories, useful for
	/// testing recursion depth handling and stack usage.
	...
	/// Create a tree with mixed file types and permissions for comprehensive
	/// testing
	///
	/// Creates files with different extensions, sizes, and permissions (on
	/// Unix). Useful for testing file type detection, permission handling, and
	/// formatting.
