...
// spell-checker:ignore gettime BOOTTIME clockid boottime nusers loadavg
// getloadavg timeval
...
/// # Arguments
///
/// boot_time: Option<time_t> - Manually specify the boot time, or None to try
/// to get it from the system.
///
/// # Returns
///
/// Returns a UResult with the uptime in seconds if successful, otherwise an
/// UptimeError.
#[cfg(target_os = "openbsd")]
pub fn get_uptime(_boot_time: Option<time_t>) -> UResult<i64> {
	use libc::{CLOCK_BOOTTIME, c_int, clock_gettime, timespec};

	let mut tp: timespec = timespec { tv_sec: 0, tv_nsec: 0 };

	// OpenBSD prototype: clock_gettime(clk_id: ::clockid_t, tp: *mut ::timespec) ->
	// ::c_int;
...
pub fn get_uptime(boot_time: Option<time_t>) -> UResult<i64> {
	...
	use crate::utmpx::{BOOT_TIME, Utmpx};
	...
	// macOS-specific fallback: use sysctl kern.boottime when utmpx did not provide
	// BOOT_TIME
	//
	// On macOS, the utmpx BOOT_TIME record can be unreliable or absent, causing
	// intermittent test failures (see issue #3621: https://github.com/uutils/coreutils/issues/3621).
	// The sysctl(CTL_KERN, KERN_BOOTTIME) approach is the canonical way to retrieve
	// boot time on macOS and is always available, making uptime more reliable on
	// this platform.
	//
...
///
/// boot_time: Option<time_t> - Manually specify the boot time, or None to try
/// to get it from the system. output_format: OutputFormat - Selects the format
/// of the output string.
///
/// # Returns
///
/// Returns a UResult with the uptime in a human-readable format(e.g. "1 day,
/// 3:45") if successful, otherwise an UptimeError.
#[inline]
...
pub fn get_nusers() -> usize {
	use crate::utmpx::{USER_PROCESS, Utmpx};
...
/// # Returns
///
/// Returns a UResult with the load average if successful, otherwise an
/// UptimeError. The load average is a tuple of three floating point numbers
/// representing the 1-minute, 5-minute, and 15-minute load averages.
#[cfg(unix)]
...
/// Get the system load average
/// Windows does not have an equivalent to the load average on Unix-like
/// systems.
///
/// # Returns
///
...
/// # Returns
///
/// Returns a UResult with the load average in a human-readable format if
/// successful, otherwise an UptimeError. e.g. "load average: 0.00, 0.00, 0.00"
#[inline]
pub fn get_formatted_loadavg() -> UResult<String> {
	...
	Ok(crate::locale::get_message_with_args("uptime-lib-format-loadavg", args))
}
...
mod tests {
	...
	/// Test that sysctl kern.boottime is accessible on macOS and returns valid
	/// boot time. This ensures the fallback mechanism added for issue #3621
	/// works correctly.
	#[test]
	#[cfg(target_os = "macos")]
	fn test_macos_sysctl_boottime_available() {
		...
		assert!(boot_time > 946_684_800, "Boot time should be after year 2000");

		// Boot time should be before current time
		let now = Timestamp::now().as_second();
		assert!((boot_time as i64) < now, "Boot time should be before current time");
	}
	...
	fn test_get_uptime_always_succeeds_on_macos() {
		...
		assert!(result.is_ok(), "get_uptime should always succeed on macOS with sysctl fallback");
		...
		assert!(uptime < 365 * 86400, "Uptime seems unreasonably high: {uptime} seconds");
	}
