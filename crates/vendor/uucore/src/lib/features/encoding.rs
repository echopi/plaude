...
impl Base64SimdWrapper {
	...
	pub fn new(
		...
		Self { alphabet, use_padding, unpadded_multiple, valid_decoding_multiple }
	}
}
...
impl EncodingWrapper {
	pub fn new(
		...
		Self { alphabet, encoding, unpadded_multiple, valid_decoding_multiple }
	}
}
...
pub trait SupportsFastDecodeAndEncode {
	...
	/// Inputs with a length that is a multiple of this number do not have
	/// padding when encoded. For instance:
	///
	/// "The quick brown"
	///
	/// is 15 characters (divisible by 3), so it is encoded in Base64 without
	/// padding:
	///
	...
	/// "The quick brown fox"
	///
	/// is 19 characters, which is not divisible by 3, so its Base64
	/// representation has padding:
	///
	/// "VGhlIHF1aWNrIGJyb3duIGZveA=="
	///
	/// The encoding performed by `fast_encode` depends on this number being
	/// correct.
	fn unpadded_multiple(&self) -> usize;

	/// Data to decode must be a length that is multiple of this number
	///
	/// The decoding performed by `fast_decode` depends on this number being
	/// correct.
	fn valid_decoding_multiple(&self) -> usize;

	/// Whether the decoder can flush partial chunks (multiples of
	/// `valid_decoding_multiple`) before seeing the full input. Defaults to
	/// `false` for encodings that must consume the entire input (e.g. base58).
	fn supports_partial_decode(&self) -> bool {
		false
	}

	/// Gives encoding-specific logic a chance to pad a trailing, non-empty
	/// remainder before the final decode attempt. The default implementation
	/// opts out.
...
impl SupportsFastDecodeAndEncode for Z85Wrapper {
	...
	fn encode_to_vec_deque(&self, input: &[u8], output: &mut VecDeque<u8>) -> UResult<()> {
		// According to the spec we should not accept inputs whose len is not a multiple
		// of 4. However, the z85 crate implements a padded encoding and accepts such
		// inputs. We have to manually check for them.
	...
	fn decode_into_vec(&self, input: &[u8], output: &mut Vec<u8>) -> UResult<()> {
		...
		match self.encoding.decode_mut(input, &mut (output[output_len..])) {
			Ok(us) => {
				...
				// "Returns the length of the decoded output. This length may be smaller than
				// the output length if the input contained padding or ignored characters. The
				// output bytes after the returned length are not initialized and should not be
				// read."
...
impl SupportsFastDecodeAndEncode for Base32Wrapper {
	...
	fn pad_remainder(&self, remainder: &[u8]) -> Option<PadResult> {
		...
		Some(PadResult { chunk: padded, had_invalid_tail: trimmed })
	}
