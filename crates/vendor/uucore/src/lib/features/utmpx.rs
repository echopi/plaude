...
// Musl implements only stubs for the utmp functions, and the libc crate issues a deprecation
// warning about this. However, calling these stubs is the correct approach to maintain
...
mod ut {
	...
	pub use libc::{
		ACCOUNTING, BOOT_TIME, DEAD_PROCESS, EMPTY, INIT_PROCESS, LOGIN_PROCESS, NEW_TIME, OLD_TIME,
		RUN_LVL, USER_PROCESS,
	};
}
...
mod ut {
	...
	pub use libc::{
		_UTX_HOSTSIZE as UT_HOSTSIZE, _UTX_IDSIZE as UT_IDSIZE, _UTX_LINESIZE as UT_LINESIZE,
		_UTX_USERSIZE as UT_NAMESIZE, ACCOUNTING, BOOT_TIME, DEAD_PROCESS, EMPTY, INIT_PROCESS,
		LOGIN_PROCESS, NEW_TIME, OLD_TIME, RUN_LVL, SHUTDOWN_TIME, SIGNATURE, USER_PROCESS,
	};
}
...
mod ut {
	...
	pub use libc::{
		BOOT_TIME, DEAD_PROCESS, EMPTY, INIT_PROCESS, LOGIN_PROCESS, NEW_TIME, OLD_TIME,
		SHUTDOWN_TIME, USER_PROCESS,
	};
}
...
mod ut {
	...
	pub use libc::{
		_UTX_HOSTSIZE as UT_HOSTSIZE, _UTX_IDSIZE as UT_IDSIZE, _UTX_LINESIZE as UT_LINESIZE,
		_UTX_USERSIZE as UT_NAMESIZE, ACCOUNTING,
	};
...
mod ut {
	...
	pub use libc::{
		BOOT_TIME, DEAD_PROCESS, INIT_PROCESS, LOGIN_PROCESS, NEW_TIME, OLD_TIME, RUN_LVL,
		USER_PROCESS, UT_HOSTSIZE, UT_IDLEN, UT_LINESIZE, UT_NAMESIZE,
	};
}
...
impl Utmpx {
	...
	pub fn canon_host(&self) -> IOResult<String> {
		...
		if !hostname.is_empty() {
			...
			let hints = AddrInfoHints { flags: AI_CANONNAME, ..AddrInfoHints::default() };
	...
	///
	/// This will use the default location, or the path
	/// [`Utmpx::iter_all_records_from`] was most recently called with.
	///
	...
	fn new_systemd() -> Self {
		...
		let systemd_iter = match systemd_logind::SystemdUtmpxIter::new() {
			...
			Err(_) => {
				...
				// Return empty iterator rather than falling back  (GNU coreutils also returns 0
				// when /var/run/utmp is not present, so we don't need to propagate the error
				// here)
				systemd_logind::SystemdUtmpxIter::empty()
			},
		};
		Self { guard, phantom: PhantomData, systemd_iter: Some(systemd_iter) }
	}
}

/// Wrapper type that can hold either traditional utmpx records or systemd
/// records
	...
	fn next(&mut self) -> Option<Self::Item> {
		#[cfg(feature = "feat_systemd_logind")]
		{
			if let Some(ref mut systemd_iter) = self.systemd_iter {
				// We have a systemd iterator - use it exclusively (never fall back to
				// traditional utmp)
		...
		unsafe {
			...
			if res.is_null() {
				...
				// makes things easier.
				Some(UtmpxRecord::Traditional(Box::new(Utmpx { inner: ptr::read(res.cast_const()) })))
			}
		}
	}
}
