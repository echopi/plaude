...
use libc::time_t;
use libc::{
	c_char, c_int, getgrgid, getgrnam, getgroups, getpwnam, getpwuid, gid_t, group, passwd, uid_t,
};
...
impl Passwd {
	...
	unsafe fn from_raw(raw: passwd) -> Self {
		Self {
			...
			uid: raw.pw_uid,
			gid: raw.pw_gid,
			#[cfg(not(all(target_os = "android", any(target_arch = "x86", target_arch = "arm"))))]
	...
	pub fn belongs_to(&self) -> Vec<gid_t> {
		...
		loop {
			ngroups_old = ngroups;
			if unsafe { getgrouplist(name.as_ptr(), self.gid, groups.as_mut_ptr(), &raw mut ngroups) }
				== -1
			{
...
impl Group {
	...
	unsafe fn from_raw(raw: group) -> Self {
		Self { name: cstr2string(raw.gr_name).expect("group without name"), gid: raw.gr_gid }
	}
}
