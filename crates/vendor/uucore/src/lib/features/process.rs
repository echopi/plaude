...
use nix::errno::Errno;
use nix::{
	sys::signal::{self as nix_signal, SigHandler, Signal},
	unistd::Pid,
};
...
/// # Error
///
/// - [Errno::EPERM] A process with process ID pid exists, but it is not in the
///   same session as the calling process, and the implementation considers this
///   an error.
...
impl ChildExt for Child {
	...
	fn send_signal_group(&mut self, signal: usize) -> io::Result<()> {
		...
		// Signal 0 is special - it just checks if process exists, doesn't send
		// anything. No need to manipulate signal handlers for it.
		if signal == 0 {
	...
	fn wait_or_timeout(
		...
		loop {
			...
			// XXX: this is kinda gross, but it's cleaner than starting a thread just to
			// wait      (which was the previous solution).  We might want to use a
			// different duration      here as well
