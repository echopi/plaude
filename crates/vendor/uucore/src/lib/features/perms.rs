...
impl Default for Verbosity {
	fn default() -> Self {
		Self { groups_only: false, level: VerbosityLevel::Normal }
	}
}
...
pub struct ChownExecutor {
	...
	pub raw_owner:         String, /* The owner of the file as input by the user in the command
	                                * line. */
...
/// In the context of chown and chgrp, check whether we are in a "preserve-root"
/// scenario.
///
...
///     )
/// The first clause is checked by the caller, the second and third clause is
/// checked here. The caller has to evaluate -P/-H/-L into
/// 'would_recurse_symlink'. Recall that canonicalization resolves both relative
/// paths (e.g. "..") and symlinks.
fn is_root(path: &Path, would_traverse_symlink: bool) -> bool {
	...
	// If we would_recurse_symlink, then the clause is true no matter whether the
	// path is a symlink or not. Otherwise, we only need to check here if the path
	// can syntactically be a symlink:
	...
	// FIXME: TOCTOU bug! canonicalize() runs at a different time than WalkDir's
	// recursion decision. However, we're forced to make the decision whether to
	// warn about --preserve-root *before* even attempting to chown the path, let
	// alone doing the stat inside WalkDir.
...
impl ChownExecutor {
	...
	fn traverse<P: AsRef<Path>>(&self, root: P) -> i32 {
		...
		let Some(meta) = self.obtain_meta(path, self.dereference) else {
			if self.verbosity.level == VerbosityLevel::Verbose {
				println!("failed to change ownership of {} to {}", path.quote(), self.raw_owner);
			}
			return 1;
		};
	...
	fn safe_dive_into<P: AsRef<Path>>(&self, root: P) -> i32 {
		...
		// This matches WalkDir's behavior with min_depth(1) - if root is not a
		// directory, there are no children to traverse, so we return early with
		// success
		if !root.is_dir() {
			return 0;
		}
	...
	fn safe_traverse_dir(&self, dir_fd: &DirFd, dir_path: &Path, ret: &mut i32) {
		...
		let entries = match dir_fd.read_dir() {
			...
			Err(e) => {
				*ret = 1;
				if self.verbosity.level != VerbosityLevel::Silent {
					show_error!("cannot read directory {}: {}", dir_path.quote(), strip_errno(&e));
				}
				return;
			},
		};
	...
	fn dive_into<P: AsRef<Path>>(&self, root: P) -> i32 {
		...
		// walkdir always dereferences the root directory, so we have to check it
		// ourselves
		if self.traverse_symlinks == TraverseSymlinks::None && root.is_symlink() {
			return 0;
		}
		...
		let mut iterator = WalkDir::new(root)
			...
			.min_depth(1)
			.into_iter();
		// We can't use a for loop because we need to manipulate the iterator inside the
		// loop.
	...
	fn print_verbose_ownership_retained_as(&self, path: &Path, uid: u32, gid: Option<u32>) {
		if self.verbosity.level == VerbosityLevel::Verbose {
			let ownership = match (self.dest_uid, self.dest_gid, gid) {
				...
				(None, Some(_), Some(gid)) => entries::gid2grp(gid).unwrap_or_else(|_| gid.to_string()),
				...
				VerbosityLevel::Changes | VerbosityLevel::Verbose => {
					let output = if self.verbosity.groups_only {
						format!(
							"changed group of {} from {} to {}",
							path.quote(),
							entries::gid2grp(original_gid).unwrap_or_else(|_| original_gid.to_string()),
							entries::gid2grp(dest_gid).unwrap_or_else(|_| dest_gid.to_string())
						)
					} else {
						format!(
							"changed ownership of {} from {}:{} to {}:{}",
							path.quote(),
							entries::uid2usr(original_uid).unwrap_or_else(|_| original_uid.to_string()),
							entries::gid2grp(original_gid).unwrap_or_else(|_| original_gid.to_string()),
							entries::uid2usr(dest_uid).unwrap_or_else(|_| dest_uid.to_string()),
							entries::gid2grp(dest_gid).unwrap_or_else(|_| dest_gid.to_string())
						)
...
/// and the filter, from `ArgMatches`.
...
pub fn chown_base(
	...
	let GidUidOwnerFilter { dest_gid, dest_uid, raw_owner, filter } =
	...
	let executor = ChownExecutor {
		...
		dest_uid,
		raw_owner,
		verbosity: Verbosity { groups_only, level: verbosity_level },
		recursive,
		dereference,
		preserve_root,
		files,
...
mod tests {
	...
	fn test_literal_root() {
		...
		assert_eq!(path.to_str(), Some("/"), "cfg(unix) but using non-unix path delimiters?!");
		// Must return true, this is the main scenario that --preserve-root shall
		// prevent.
		assert!(is_root(&path, false));
		assert!(is_root(&path, true));
	}
