...
impl Formatter<i64> for SignedInt {
	fn fmt(&self, writer: impl Write, x: i64) -> std::io::Result<()> {
		// -i64::MIN is actually 1 larger than i64::MAX, so we need to cast to i128
		// first.
	...
	fn try_from_spec(s: Spec) -> Result<Self, FormatError> {
		let Spec::SignedInt { width, precision, positive_sign, alignment, position: _position } = s
		else {
		...
		Ok(Self { width, precision, positive_sign, alignment })
	}
}
...
impl Formatter<u64> for UnsignedInt {
	...
	fn try_from_spec(s: Spec) -> Result<Self, FormatError> {
		// A signed int spec might be mapped to an unsigned int spec if no sign is
		// specified
		...
		let Spec::UnsignedInt { variant, width, precision, alignment, position: _position } = s
		else {
			return Err(FormatError::WrongSpecType);
		};
		...
		Ok(Self { variant, width, precision, alignment })
	}
}
...
impl Formatter<&ExtendedBigDecimal> for Float {
	fn fmt(&self, writer: impl Write, e: &ExtendedBigDecimal) -> std::io::Result<()> {
		/* TODO: Might be nice to implement Signed trait for ExtendedBigDecimal (for
		 * abs) at some point, but that requires implementing a _lot_ of traits.
		...
		let s = if let ExtendedBigDecimal::BigDecimal(bd) = abs {
			match self.variant {
				FloatVariant::Decimal => format_float_decimal(&bd, self.precision, self.force_decimal),
		...
		Self: Sized,
	{
		...
		Ok(Self { variant, case, force_decimal, width, positive_sign, alignment, precision })
	}
}
...
fn bd_to_string_exp_with_prec(bd: &BigDecimal, precision: usize) -> (String, i64) {
	...
	// (e.g. 9995.with_prec(3) => 1000 * 10^1, but we want 100 * 10^2), we
...
fn format_float_shortest(
	...
	if exponent < -4 || exponent >= precision as i64 {
		...
		//  - The precision works differently and specifies the total number of digits
		//    instead of the digits in the fractional part.
...
fn format_float_hexadecimal(
	...
	if BigDecimal::zero().eq(bd) {
		// To print 0, we don't ever need any digits after the decimal point, so default
		// to that if precision is not specified.
	...
	let (mut frac2, mut exp2) = if exp10 >= 0 {
		...
		// rounding.
		let margin = ((max_precision + 1) as i64 * 4 - frac10.bits() as i64).max(0) + -exp10 * 3 + 1;
	...
	// Emulate x86(-64) behavior, we display 4 binary digits before the decimal
	// point, so the value will always be between 0x8 and 0xf.
	...
	// Convert "XXX" to "X.XX": that divides by 16^precision = 2^(4*precision), so
	// add that to the exponent.
...
fn write_output(
	...
	// Take length of `sign_indicator`, which could be 0 or 1, into consideration
	// when padding by storing remaining_width indicating the actual width needed.
...
mod test {
	...
	fn decimal_float() {
		...
		let f =
			|x| format_float_decimal(&BigDecimal::from_f64(x).unwrap(), Some(0), ForceDecimal::Yes);
		assert_eq!(f(100.0), "100.");

		// Test arbitrary precision: long inputs that would not fit in a f64, print 24
		// digits after decimal point.
		let f =
			|x| format_float_decimal(&BigDecimal::from_str(x).unwrap(), Some(24), ForceDecimal::No);
		assert_eq!(f("0.12345678901234567890"), "0.123456789012345678900000");
		assert_eq!(f("1234567890.12345678901234567890"), "1234567890.123456789012345678900000");
	}
	...
	fn decimal_float_zero() {
		use super::format_float_decimal;
		let f = |digits, scale| {
			format_float_decimal(&BigDecimal::from_bigint(digits, scale), Some(6), ForceDecimal::No)
		};
	...
	fn scientific_float() {
		...
		// Test "0e10"/"0e-10". From cppreference.com: "If the value is ​0​, the exponent
		// is also ​0​."
		let f = |digits, scale| {
			format_float_scientific(
	...
	// Some end-to-end tests, `printf` will also test some of those but it's easier
	// to add more tests here. We mostly focus on padding, negative numbers, and
	// format specifiers that are not covered above.
	#[test]
	...
	fn format_signed_int_precision_zero() {
		...
		assert_eq!(fmt(&format, 123i64), "123");
		// From cppreference.com: "If both the converted value and the precision are ​0​
		// the conversion results in no characters."
	...
	fn format_float_decimal() {
		...
		// We've had issues with "0e10"/"0e-10" formatting, and our current workaround
		// is in Format.fmt function.
