...
use crate::{NonUtf8OsStrError, error::UError, extendedbigdecimal::ExtendedBigDecimal};
...
impl Display for FormatError {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			Self::SpecError(s) => {
				write!(f, "%{}: invalid conversion specification", String::from_utf8_lossy(s))
			},
			Self::TooManySpecs(s) => {
				write!(f, "format '{}' has too many % directives", String::from_utf8_lossy(s))
			},
...
/// Rust's formatter will panic when trying to allocate memory for very large
/// widths. This limit is somewhat arbitrary but should be well above any
/// practical use case while still preventing formatter panics.
...
fn check_width(width: usize) -> std::io::Result<()> {
	if width > MAX_FORMAT_WIDTH {
		Err(std::io::Error::new(std::io::ErrorKind::OutOfMemory, "formatting width too large"))
	} else {
		Ok(())
	}
}
...
///
/// ```rust
/// use uucore::format::{FormatArgument, printf};
///
...
///
/// ```rust
/// use uucore::format::{FormatArgument, sprintf};
///
...
///
/// This is used by `seq` and `csplit`. It can be constructed with
/// [`Format::from_formatter`] or [`Format::parse`] and can write a value with
/// [`Format::fmt`].
///
/// [`Format::parse`] can only accept a single specification without any
/// asterisk parameters. If it does get more specifications, it will return an
/// error.
...
impl<F: Formatter<T>, T> Format<F, T> {
	pub fn from_formatter(formatter: F) -> Self {
		Self { prefix: Vec::<u8>::new(), suffix: Vec::<u8>::new(), formatter, _marker: PhantomData }
	}
	...
	pub fn parse(format_string: impl AsRef<[u8]>) -> Result<Self, FormatError> {
		...
		Ok(Self { prefix, suffix, formatter, _marker: PhantomData })
	}
