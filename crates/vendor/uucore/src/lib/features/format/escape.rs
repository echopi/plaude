	...
	if let [c, new_rest @ ..] = rest {
		...
		*rest = new_rest;
		match c {
			...
			b'0' => {
				Ok(EscapedChar::Byte(parse_code(rest, Base::Oct(zero_octal_parsing)).unwrap_or(b'\0')))
			},
...
mod tests {
	...
	mod parse_unicode {
		...
		fn no_characters() {
			let input = b"";
			assert_eq!(parse_unicode(&mut &input[..], 8), Err(EscapeError::MissingHexadecimalNumber));
		}

		#[test]
		fn incomplete_hexadecimal_number() {
			let input = b"123";
			assert_eq!(parse_unicode(&mut &input[..], 4), Err(EscapeError::MissingHexadecimalNumber));
		}
