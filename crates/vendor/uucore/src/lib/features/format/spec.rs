...
/// Precision and width specified might use an asterisk to indicate that they
/// are determined by an argument.
#[derive(Clone, Copy, Debug)]
		...
		while let Some(x) = rest.get(*index) {
			match x {
				...
				b'\'' => {
					...
					// this is a no-op in the "C" locale. We only save this flag for reporting
					// errors
					flags.quote = true;
				},
				_ => break,
		...
		*rest = &start[index..];
		...
		Ok(match type_spec {
			...
			b'c' => {
				...
				Self::Char { position, width, align_left: flags.minus }
			},
			b's' => {
				if flags.zero || flags.hash || flags.quote {
					return Err(&start[..index]);
				}
				Self::String { position, precision, width, align_left: flags.minus }
			},
			b'b' => {
			...
			b'd' | b'i' => {
				...
				Self::SignedInt { position, width, precision, alignment, positive_sign }
			},
			c @ (b'u' | b'o' | b'x' | b'X') => {
				...
				Self::UnsignedInt { position, variant, precision, width, alignment }
			},
	...
	pub fn write(
		...
		match self {
			Self::Char { width, align_left, position } => {
				...
				write_padded(writer, &[args.next_char(*position)], width, *align_left || neg_width)
			},
			...
			Self::QuotedString { position } => {
				let s =
					locale_aware_escape_name(args.next_string(*position), QuotingStyle::SHELL_ESCAPE);
			...
			Self::Float {
				...
				if precision.is_some_and(|p| p as u64 > i32::MAX as u64) {
					return Err(FormatError::InvalidPrecision(precision.unwrap().to_string()));
				}
...
mod tests {
	...
	mod resolve_asterisk_width {
		...
		#[test]
		fn no_width() {
			assert_eq!(None, resolve_asterisk_width(None, &mut FormatArguments::new(&[])));
		}
		...
		fn fixed_width() {
			assert_eq!(
				Some((42, false)),
				resolve_asterisk_width(Some(CanAsterisk::Fixed(42)), &mut FormatArguments::new(&[]))
			);
		}
		...
		fn asterisks_with_numbers() {
			...
			assert_eq!(
				Some((2, false)),
				resolve_asterisk_width(
					Some(CanAsterisk::Asterisk(ArgumentLocation::Position(NonZero::new(2).unwrap()))),
	...
	mod resolve_asterisk_precision {
		...
		#[test]
		fn no_width() {
			assert_eq!(None, resolve_asterisk_precision(None, &mut FormatArguments::new(&[])));
		}
