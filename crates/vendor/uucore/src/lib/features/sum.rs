...
impl Blake2b {
	...
	pub fn with_output_bytes(output_bytes: usize) -> Self {
		...
		Self { digest: state, bit_size: output_bytes * 8 }
	}
}
...
impl Blake3 {
	...
	pub fn with_output_bytes(output_bytes: usize) -> Self {
		Self { digest: blake3::Hasher::new(), byte_size: output_bytes }
	}
}

impl Default for Blake3 {
	fn default() -> Self {
		Self { digest: blake3::Hasher::default(), byte_size: Self::DEFAULT_BYTE_SIZE }
	}
}
...
impl Default for Crc {
	fn default() -> Self {
		Self { digest: crc_fast::Digest::new_with_params(Self::get_posix_cksum_params()), size: 0 }
	}
}
...
impl Default for CRC32B {
	fn default() -> Self {
		Self { digest: crc_fast::Digest::new(crc_fast::CrcAlgorithm::Crc32IsoHdlc) }
	}
}
	...
	($algo_type:ty, $default_output_bits:literal) => {
		impl $algo_type {
			...
			pub fn with_output_bits(bits: usize) -> Self {
				Self { digest: Default::default(), bit_size: bits }
			}
		}
		impl Default for $algo_type {
			fn default() -> Self {
...
impl<'a> DigestWriter<'a> {
	pub fn new(digest: &'a mut Box<dyn Digest>, binary: bool) -> Self {
		...
		DigestWriter { digest, binary, was_last_character_carriage_return }
	}
...
mod tests {
	...
	fn test_crlf_across_blocks() {
		...
		use super::{Digest, DigestWriter, Md5};
	...
	fn test_crc_known_values() {
		...
		let test_cases =
			[("", 4_294_967_295_u64), ("a", 1_220_704_766_u64), ("abc", 1_219_131_554_u64)];
