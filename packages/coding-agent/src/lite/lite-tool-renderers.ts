...
import { isClaudeStyle } from "./render-policy";
...
function renderLine(text: string, theme: Theme, color: "dim" | "text" | "error" = "text"): Component {
	return new Text(theme.fg(color, truncateToWidth(text, 120)), isClaudeStyle() ? 2 : 0, 0);
}
...
function renderDiffLine(line: string, theme: Theme): Component {
	...
	return new Text(theme.fg(color, truncateToWidth(line, 120)), isClaudeStyle() ? 2 : 0, 0);
}
